...
- Follow guidelines at: [openHAB Developer Guidelines](https://www.openhab.org/docs/developer/guidelines.html)
- Follow checklist at: [Review Checklist](https://github.com/openhab/openhab-addons/wiki/Review-Checklist)
...
- Reference documentation: [bndtools instructions](https://bnd.bndtools.org/chapters/825-instructions-ref.html)
...
- Reference documentation: [i18n documentation](https://www.openhab.org/docs/developer/utils/i18n.html#managing-translations)
...
1. Choose a unique binding name following naming conventions
...
- Reference documentation: [Add-on XML structure](https://www.openhab.org/docs/developer/addons/addon.html#xml-structure-for-add-on-definitions)

### Creation Process

Important: Run all the following commands from repo root folder.

Check the [documentation for how to create new bindings](https://www.openhab.org/docs/developer/#develop-a-new-binding).

Ask the user to name the binding `<bindingname>` and use the supplied name.
It needs to be in CamelCase and match `[A-Z][A-Za-z]*`.

Make sure that `bundles/org.openhab.binding.<bindingname>` does not exist.

Do not create folder contents on your own.
Run the script `create_openhab_binding_skeleton` at project root directory with sh or powershell to create.
`authorname` needs to be in `""` as it can contain spaces.
`githubusername` is `[a-z0-9]*`.
The authorname may be deduced from git config.

Run `mvn spotless:apply` in the new folder.
Make sure you don't run this at top directory but in the binding folder to avoid long runtime.
...
1. **Manual Creation:** Don't create binding structures manually - use the skeleton scripts
1. **Import Order:** Unsorted imports will fail style checks
1. **Java Version:** Stay within Java 21 - newer features will break CI/CD

## Getting Help

- **Documentation:** [openHAB Developer Docs](https://www.openhab.org/docs/developer/)
- **Community Forum:** [community.openhab.org](https://community.openhab.org/)
