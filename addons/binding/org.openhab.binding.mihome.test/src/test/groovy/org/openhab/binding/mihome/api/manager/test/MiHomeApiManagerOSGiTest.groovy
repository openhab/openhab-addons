...
import static org.openhab.binding.mihome.internal.api.manager.MiHomeApiManagerImpl.*
...
import org.openhab.binding.mihome.test.FailingRequestHandlerMock
...
/**
 * Test class for {@link MiHomeApiManager}
 *
 * @author Svilen Valkanov
 */

@RunWith(Parameterized.class)
class MiHomeApiManagerOSGiTest extends OSGiTest {
    ...
    private Logger logger = LoggerFactory.getLogger(MiHomeApiManagerOSGiTest.class)
    ...
    HttpServlet servlet
    static MiHomeApiManager apiManager

    public MiHomeApiManagerOSGiTest(def path, def requestContent, Callback callback, def successfullRequest, def HTTPRequestFailed, def JsonRequestFailed, def IOExceptionCaught) {
        this.path = path;
    ...
    private MiHomeApiManager configureApiManager() {
        // Configure the MiHomeApiManager
        MiHomeApiConfiguration config = new MiHomeApiConfiguration(TEST_USERNAME, TEST_API_KEY);
        ...
        return new MiHomeApiManagerImpl(config, client, failingRequestHandler);
    }
    ...
    static <T> T getServiceFromStaticContext(Class<T> clazz) {
        def bundle = FrameworkUtil.getBundle(MiHomeApiManagerOSGiTest);
