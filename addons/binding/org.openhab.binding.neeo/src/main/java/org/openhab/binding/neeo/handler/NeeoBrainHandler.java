...
public class NeeoBrainHandler extends AbstractBridgeHandler {
     ...
     */
    private final AtomicReference<ScheduledFuture<?>> initializationTask = new AtomicReference<>(null);
     ...
     */
    NeeoBrainHandler(@NonNull Bridge bridge, int servicePort, HttpService httpService,
    ...
    public void initialize() {
        NeeoUtil.cancel(initializationTask.getAndSet(scheduler.schedule(() -> {
            initializeTask();
        }, 1, TimeUnit.SECONDS)));
    }
        ...
        try {
            ...
            if (config.isEnableForwardActions()) {
                ...
                try {
                    ...
                    final URL url = new URL(createCallbackUrl(brainId, config), path);
    ...
    private URL createCallbackUrl(String brainId, NeeoBrainConfig config) throws MalformedURLException {
        ...
        try {
            try {
                final byte[] mask = { -1, -1, -1, 0 }; // 255.255.255.0
                final byte[] brainBytes = InetAddress.getByName(config.getIpAddress()).getAddress();
                ...
                while (interfaces.hasMoreElements()) {
                    ...
                    while (addresses.hasMoreElements()) {
                        ...
                        boolean match = true;
                        for (int i = 0; i < brainBytes.length; i++) {
                            if ((brainBytes[i] & mask[i]) != (addrBytes[i] & mask[i])) {
                                match = false;
                            }
                        }

                        if (match) {
                            return new URL("http://" + addr.getHostAddress() + ":" + servicePort);
                        }
                    }
                }
            } catch (UnknownHostException e) {
                logger.debug("Couldn't look up ipaddress: {}: {}", config.getIpAddress(), e.getMessage());
            }
        ...
        final String localIpAddress = config.getLocalIpAddress();
        if (localIpAddress != null) {
            try {
                final String ipAddress = localIpAddress.trim();
