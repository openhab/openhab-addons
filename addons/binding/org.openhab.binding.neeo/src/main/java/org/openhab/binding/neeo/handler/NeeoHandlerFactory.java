 ...
 */
@Component(service = org.eclipse.smarthome.core.thing.binding.ThingHandlerFactory.class, immediate = true)
public class NeeoHandlerFactory extends BaseThingHandlerFactory {
     ...
     */
    @Reference(cardinality = ReferenceCardinality.MANDATORY, service = HttpService.class, name = "HttpService", policy = ReferencePolicy.STATIC, unbind = "unsetHttpService")
     ...
     *
     * @param httpService the non-null {@link NetworkAddressService} to use
     */
    @Reference(cardinality = ReferenceCardinality.MANDATORY, service = NetworkAddressService.class, name = "NetworkServiceAddress", policy = ReferencePolicy.STATIC, unbind = "unsetNetworkAddressService")
     ...
     * @param httpService the non-null {@link NetworkAddressService} to use
     */
    @Reference(cardinality = ReferenceCardinality.MANDATORY, service = DiscoveryServiceRegistry.class, name = "DiscoveryServiceRegistry", policy = ReferencePolicy.STATIC, unbind = "unsetDiscoveryServiceRegistry")
    ...
    protected ThingHandler createHandler(Thing thing) {
        ...
        if (thingTypeUID.equals(BRIDGE_TYPE_BRAIN)) {
            ...
            startScan(2000); // starts scan for rooms
            return new NeeoBrainHandler((Bridge) thing, port < 0 ? 8080 : port, httpService, networkAddressService);
        } else if (thingTypeUID.getId().startsWith("room")) {
            startScan(1000); // starts scan for devices
    ...
    private void startScan(int waitMilliSeconds) {
        ...
        scheduler.schedule(() -> {
            discoveryServiceRegistry.startScan(NeeoConstants.BINDING_ID, null);
