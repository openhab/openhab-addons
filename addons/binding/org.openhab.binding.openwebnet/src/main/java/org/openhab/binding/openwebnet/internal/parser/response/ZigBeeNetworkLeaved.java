 ...
 */
@NonNullByDefault
public class ZigBeeNetworkLeaved extends Response {

    @SuppressWarnings("null")
    private final Logger logger = LoggerFactory.getLogger(ZigBeeNetworkLeaved.class);

    @Override
    public boolean check(String message) {
        return message.matches("\\*13\\*34\\*[0-9]+#9##");
    }
    ...
    public void process(String message, ResponseListener e) {
        ...
        try {
            ...
            e.onNetworkLeave(where);
        } catch (NumberFormatException e2) {
            // open network with negative value
            logger.warn("Network leaved by a source that cannot be parsed ({})", message);
        }
    }
}
