 ...
 */
@NonNullByDefault
public class SupervisorOff extends Response {

    @SuppressWarnings("null")
    private final Logger logger = LoggerFactory.getLogger(SupervisorOff.class);

    @Override
    public boolean check(String message) {
        return message.matches("\\*13\\*67\\*[0-9]+#9##");
    }
    ...
    public void process(String message, ResponseListener e) {
        ...
        try {
            ...
            logger.warn("Supervisor On but the source cannot be parsed ({})", message);
            where = -1;
        }
        e.onSupervisorOff(where);
    }
}
