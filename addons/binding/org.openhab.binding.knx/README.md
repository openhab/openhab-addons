...
The *ets* gateway is very similar to the *ip* gateway in the sense that it has the same configuration parameters, with addition of the following parameters

| Name                | Required | Description                                                                                                  | Default value |
|---------------------|----------|--------------------------------------------------------------------------------------------------------------|---------------|
| knxProj          | Y        | The ETS 5 .knxproj file to parse                                                        | -             |

For each Individual Address that is defined in the *knxProj* file, a *generic* Thing will be created and provided to the runtime.
...
| pingInterval | N        | Interval (in seconds) to contact the device and set the thing status based on the result (requires the address)  (0 if the device should not be polled)         | 600                                                                         |
...
*generic* Things are just like the *basic* Things wrappers around an group address, but they are generated by the ETS knxproj parser, and they can only be associated to a *ets* Bridge. In addition, the *readInterval* parameter will be set to 3600 seconds, and the *pingInterval* is set to 0

The *generic* Thing will be named x_y_z if the physical address of the device is x.y.z
...
When using the *ets* Bridge thing the knxproj parser will automatically create a *generic* Channel for each Group Address that is by a given *generic* Thing that was discovered in the knxproj file. The Type of the Channel will automatically be set to an Item Type that is compatible with the Datapoint type (DPT) that is defined in the knxproj file 

The *generic* Channel will be named x_y_z based if the Group Address is x/y/z
...
The binding provides two new *StateProfile* that can be used specifically in conjunction with Items that are linked to *generic* Channels. 

##### Control Profile

The *control* profile forwards Commands from Items to the Handler, and the other way around, but does not react upon State updates.

##### Listen Profile

The *listen* profile forwards Commands ands States originating from the Handler to the Item, but does not react upon Commands and States originating from the Item. On top of that, Commands are transformed into States before being forwarded to the Item.
...
Bridge knx:ets:etsbridge [ 
    ...
    autoReconnectPeriod=1,
    localSourceAddr="0.0.0"
    knxProj="20180209.knxproj"
]
```

The result of the parsing will look similar to:

```2018-03-28 16:25:08.218 [INFO ] [.i.a.p.KNXProjectThingProvider:277  ] - Added a Thing knx:generic:etsbridge:1_1_131 for an actor of type AKD-0401.01 Dimming actuator 4-f, 8TE, 230VAC,250W made by MDT technologies
2018-03-28 16:25:08.249 [INFO ] [.i.a.p.KNXProjectThingProvider:332  ] - Added the Channel knx:generic:etsbridge:1_1_131:1_2_65 (Type Switch, DPT 1.001, false/true/false/false, 'Dining Room - Uplighters - Switch', '1.001:1/2/65+W')
2018-03-28 16:25:08.238 [INFO ] [.i.a.p.KNXProjectThingProvider:332  ] - Added the Channel knx:generic:etsbridge:1_1_131:1_2_66 (Type Dimmer, DPT 3.007, false/true/false/false, 'Dining Room - Uplighters - Dimmer', '3.007:1/2/66+W')
2018-03-28 16:25:08.305 [INFO ] [.i.a.p.KNXProjectThingProvider:332  ] - Added the Channel knx:generic:etsbridge:1_1_131:1_2_67 (Type Dimmer, DPT 5.001, false/true/false/false, 'Dining Room - Uplighters - Dimmer', '5.001:1/2/67+W')
2018-03-28 16:25:08.284 [INFO ] [.i.a.p.KNXProjectThingProvider:332  ] - Added the Channel knx:generic:etsbridge:1_1_131:1_2_69 (Type Number, DPT 5.010, true/false/true/false, 'Dining Room - Uplighters - State Dim', '5.010:1/2/69+R+T')
```
...
Dimmer        demoParsedDimmer   "Dimmer [%d %%]" <light>                    { channel="knx:generic:etsbridge:1_1_131:1_2_65" [profile="knx:control"], channel="knx:generic:etsbridge:1_1_131:1_2_66" [profile="knx:control"], channel="knx:generic:etsbridge:1_1_131:1_2_67" [profile="knx:control"], channel="knx:generic:etsbridge:1_1_131:1_2_69"  [profile="knx:listen"]}
```
