...
public abstract class MySensorsBridgeConnection extends Thread {
	...
	public List<MySensorsMessage> MySensorsMessageOutboundQueue = Collections.synchronizedList(new LinkedList<MySensorsMessage>());
	...
	public void removeMySensorsOutboundMessage(MySensorsMessage msg) {
		...
		while(iterator.hasNext()) {
			...
			if(msgInQueue.getNodeId() == msg.getNodeId()
			&& msgInQueue.getChildId() == msg.getChildId()
			&& msgInQueue.getMsgType() == msg.getMsgType()
			&& msgInQueue.getSubType() == msg.getSubType()
			&& msgInQueue.getAck() == msg.getAck()
