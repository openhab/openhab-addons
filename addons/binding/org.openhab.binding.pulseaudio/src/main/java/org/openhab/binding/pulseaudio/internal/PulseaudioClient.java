...
public class PulseaudioClient {
	...
	public void update() {
		...
		logger.debug("Pulseaudio server "+host+": "+modules.size()+" modules and "+items.size()+" items updated");
	}
	...
	public void setCombinedSinkSlaves(Sink combinedSink, List<Sink>sinks) {
		...
		_sendRawCommand(CMD_LOAD_MODULE+" "+MODULE_COMBINE_SINK+" sink_name="+combinedSink.getPaName()+" slaves="+StringUtils.join(slaves,","));
	...
	public void moveSinkInput(SinkInput sinkInput, Sink sink) {
		if (sinkInput==null || sink==null) return;
	...
	public void moveSourceOutput(SourceOutput sourceOutput, Source source) {
		if (sourceOutput==null || source==null) return;
