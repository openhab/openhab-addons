...
public class MagentaTVControl {
     ...
     * @throws Exception
     */
    public String authenticateUser(String accountName, String accountPassword) throws Exception {
     ...
     * @throws Exception
     */
    public boolean checkDev() throws Exception {
     ...
     * @throws Exception
     */
    public void subscribePairingChannel() throws Exception {
        String sid = "";
        logger.debug("Subscribe Paring Channel for device '{}' (deviceID={}, {}:{})", thingConfig.getFriendlyName(),
    ...
    public boolean sendPairingRequest() throws Exception {
        ...
        logger.debug("Pairing initiated for device '{}' (deviceID={}).", thingConfig.getFriendlyName(),
    ...
    public boolean generateVerificationCode(String pairingCode) {
        if (thingConfig.getPairingCode().equals(pairingCode) && !thingConfig.getPairingCode().isEmpty()) {
            logger.debug("Pairing code ({}) refreshed for device {} ({}), verificationCode={}", pairingCode,
        ...
        logger.debug("verificationCode for device '{}' ({}): Input='{}', code='{}'", thingConfig.getFriendlyName(),
     ...
     * @throws Exception
     */
    public boolean verifyPairing() throws Exception {
        logger.debug("Verify pairing for device '{}' (Code='{}'", thingConfig.getFriendlyName(),
                thingConfig.getTerminalID());
        ...
        if (!result.equals("0")) {
            logger.info("Pairing failed or pairing no longer valid, result=" + result);
            resetPairing();
        ...
        if (!thingConfig.isMR400()) {
            ...
            logger.info("Features for device '{}': Enable4K:{}, EnableSAT:{}", thingConfig.getFriendlyName(), enable4K,
                    enableSAT);
        }
        return true;
    }
     ...
     * @throws Exception
     */
    public boolean sendKey(String keyName) throws Exception {

        String keyCode = getKeyCode(keyName);
        logger.debug("Send Key '{}' (keyCode='{}') to device '{}' ({})", keyName, keyCode,
                thingConfig.getFriendlyName(), thingConfig.getTerminalID());
        if (keyCode.length() <= "0x".length()) {
            logger.info("Key '{}' is unkown!", keyCode);
            return false;
        }
        ...
        String soapXml = MessageFormat.format(SOAP_ENVELOPE, soapBody);
        logger.debug("sendKey keyCode='{}' to '{}' ({}, {}:{})", keyCode, thingConfig.getFriendlyName(),
                thingConfig.getTerminalID(), thingConfig.getIpAddress(), thingConfig.getPort());
        logger.trace("sendKey terminalid='{}', pairingCode='{}', verificationCode='{}', userId={}'",
                ...
                thingConfig.getUserID());
        String response = http.httpPOST(buildHost(), buildReceiverUrl(SENDKEY_URI), soapXml, SENDKEY_SOAP_ACTION,
                CONNECTION_CLOSE);
        // Exception if request failed (response code != HTTP_OK)
        // pairingCode will be received by the Servlet, is calls onPairingResult()
        return true;
