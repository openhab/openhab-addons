...
@Component(service = UpnpDiscoveryParticipant.class, immediate = true, configurationPid = "binding." + BINDING_ID)
public class MagentaTVDiscoveryParticipant implements UpnpDiscoveryParticipant {
    ...
    public @Nullable DiscoveryResult createResult(RemoteDevice device) {
        ...
        try {
            ...
            if (uid != null) {
                ...
                logger.debug("Create Thing for device '{}' with UDN '{}, Model'{}'",
                        device.getDetails().getFriendlyName(), device.getIdentity().getUdn().getIdentifierString(),
                        device.getDetails().getModelDetails().getModelName());
    ...
    public void setMagentaTVHandlerFactory(MagentaTVHandlerFactory handlerFactory) {
        if (handlerFactory != null) {
            ...
            logger.debug("HandlerFactory bound to TMagentaTVDiscoveryParticipant");
        }
    }
