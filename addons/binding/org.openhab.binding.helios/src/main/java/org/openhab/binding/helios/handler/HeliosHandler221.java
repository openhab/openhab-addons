                            ...
                            switch (event.event) {
                                ...
                                case AUDIOLOOPTEST: {
                                    if (event.params.get("result").getAsString().equals("passed")) {
                                        ...
                                        updateState(new ChannelUID(getThing().getUID(), AUDIO_LOOP_TEST),
                                                OnOffType.OFF);
                                    } else {
                                        updateState(new ChannelUID(getThing().getUID(), AUDIO_LOOP_TEST),
                                                UnDefType.UNDEF);
                                ...
                                case NOISEDETECTED: {
                                    if (event.params.get("state").getAsString().equals("in")) {
                                        updateState(new ChannelUID(getThing().getUID(), NOISE), OnOffType.ON);
                                    } else if (event.params.get("state").getAsString().equals("out")) {
                                        updateState(new ChannelUID(getThing().getUID(), NOISE), OnOffType.OFF);
                                    } else {
                                        updateState(new ChannelUID(getThing().getUID(), NOISE), UnDefType.UNDEF);
                                    }

                                    updateState(new ChannelUID(getThing().getUID(), NOISE_STAMP), stampType);
                                    break;
                                }
                                case KEYPRESSED: {
                                ...
                                case SWITCHSTATECHANGED: {
                                    if (event.params.get("state").getAsString().equals("true")) {
                                        updateState(new ChannelUID(getThing().getUID(), SWITCH_STATE), OnOffType.ON);
                                    } else if (event.params.get("state").getAsString().equals("false")) {
                                        updateState(new ChannelUID(getThing().getUID(), SWITCH_STATE), OnOffType.OFF);
                                    } else {
                                        updateState(new ChannelUID(getThing().getUID(), SWITCH_STATE), UnDefType.UNDEF);
                                    }

                                    if (event.params.get("originator") != null) {
                                        StringType originatorType = new StringType(
                                                event.params.get("originator").getAsString());
                                        updateState(new ChannelUID(getThing().getUID(), SWITCH_STATE_ORIGINATOR),
                                                originatorType);
                                    }

                                    DecimalType switchType = new DecimalType(event.params.get("switch").getAsString());
                                    updateState(new ChannelUID(getThing().getUID(), SWITCH_STATE_SWITCH), switchType);

                                    updateState(new ChannelUID(getThing().getUID(), SWITCH_STATE_STAMP), stampType);
                                    break;
                                }
                                default: {
