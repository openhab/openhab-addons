...
public abstract class DSCAlarmBaseBridgeHandler extends BaseBridgeHandler {
	...
	public void setBridgeStatus(boolean isOnline) {
		logger.debug("setBridgeConnection(): Setting Bridge to {}", isOnline ? ThingStatus.ONLINE : ThingStatus.OFFLINE);
	...
	public void checkThings() {
		...
		for (Thing thing : things) {
			...
			if (handler != null) {
				logger.debug("***Checking '{}' - Status: {}, Initialized: {}", thing.getUID(), thing.getStatus(), handler.isThingHandlerInitialized());
	...
	public synchronized void handleIncomingMessage(String incomingMessage) {
		if (incomingMessage != null && !incomingMessage.isEmpty()) {
			...
			logger.debug("handleIncomingMessage(): Message received: {} - {}", incomingMessage, dscAlarmMessage.toString());
	...
	public boolean sendCommand(DSCAlarmCode dscAlarmCode, String... dscAlarmData) {
		...
		case KeyStroke: /* 070 */
			if (dscAlarmProtocol.equals(DSCAlarmProtocol.ENVISALINK_TPI)) {
				...
				if (dscAlarmData[0] == null || dscAlarmData[0].length() != 1 || !dscAlarmData[0].matches("[0-9]|\\*|#|F|A|P|[a-e]|<|>|=|\\^|L")) {
					logger.error("sendCommand(): \'keystroke\' must be a single character string from 0 to 9, *, #, F, A, P, a to e, <, >, =, or ^, it was: {}", dscAlarmData[0]);
					break;
