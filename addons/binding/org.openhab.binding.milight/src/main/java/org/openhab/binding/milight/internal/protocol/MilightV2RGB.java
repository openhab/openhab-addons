...
public class MilightV2RGB extends AbstractBulbInterface {
    ...
    public void setHSB(int hue, int saturation, int brightness, MilightThingState state) {
        if (saturation < 50) {
            ...
            setPower(true, state);
            state.hue360 = hue;
            sendQueue.queueRepeatable(uidc(CAT_COLOR_SET),
                    new byte[] { 0x20, MilightV3.make_color(hue), 0x55 });
    ...
    public void changeBrightness(int relative_brightness, MilightThingState state) {
        ...
        if (state.brightness != -1 && newPercent == 0) {
            ...
            for (int s = 0; s < steps; ++s) {
                sendQueue.queue(QueuedSend.NO_CATEGORY,
                        new byte[] { (byte) (relative_brightness < 0 ? 0x24 : 0x23), 0x00, 0x55 });
            }
        }
    ...
    public void previousAnimationMode(MilightThingState state) {
        setPower(true, state);
        sendQueue.queue(uidc(CAT_MODE_SET), new byte[] { 0x28, 0x00, 0x55 });
    ...
    public void nextAnimationMode(MilightThingState state) {
        setPower(true, state);
        sendQueue.queue(uidc(CAT_MODE_SET), new byte[] { 0x27, 0x00, 0x55 });
