...
import org.openhab.binding.atlona.internal.SocketSessionCallback;
...
class AtlonaPro3PortocolHandler {
     ...
     */
    private final AtlonaPro3HandlerCallback _callback;
    ...
    // private final static String CMD_RS232_FORMAT = "RS2323zone%d[%s]";
    ...
    private final Pattern _portMirrorPattern = Pattern.compile("MirrorHdmi(\\d+) Out(\\d+)");
    ...
    // private final Pattern _rs232Pattern = Pattern.compile("RS232zone(\\d+)[(.+)]");
     ...
     * @param callback a non-null {@link AtlonaPro3HandlerCallback} to update state and status
     */
    AtlonaPro3PortocolHandler(SocketSession session, AtlonaPro3Config config, AtlonaPro3Capabilities capabilities,
            AtlonaPro3HandlerCallback callback) {
    ...
    String login() throws Exception {
        ...
        logger.info("Logging into atlona switch");
        ...
        if (response.startsWith(RSP_FAILED)) {
            logger.info("Altona didn't require a login");
            postLogin();
            return null;
        }
        ...
        if (response.equals(RSP_LOGIN)) {
            if (_config.getUserName() == null || _config.getUserName().trim().length() == 0) {
                return "Atlona PRO3 has enabled IPLogin but no username was provided in the configuration.";
            }
        ...
        if (_config.getPassword() == null || _config.getPassword().trim().length() == 0) {
            return "Atlona PRO3 has enabled IPLogin but no password was provided in the configuration.";
        }
    ...
    private void postLogin() {
        logger.info("Atlona switch now connected");
    ...
    void refreshAll() {
        ...
        if (_version == null) {
            ...
            _callback.stateChanged(AtlonaPro3Constants.CHANNEL_VERSION, new StringType(_version));
        }

        if (_modelType == null) {
            refreshType();
        } else {
            _callback.stateChanged(AtlonaPro3Constants.CHANNEL_TYPE, new StringType(_modelType));
        }
    ...
    /**
     * Refresh the panel lock status. There is no command in the protocol that allows us to find out the current lock
     * status - we just assume {@link OnOffType#OFF}
     */
    void refreshPanelLock() {
        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_PANELLOCK, OnOffType.OFF);
    ...
    /**
     * Refreshes the status of the reset ports channel. There is no value for this since it's a simple command on the
     * switch - we just assume {@link OnOffType#OFF}
     */
    void refreshResetAllPorts() {
        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_RESETPORTS, OnOffType.OFF);
    ...
    /**
     * Refreshes the all ports channel. There is no state on this - simply return 1
     */
    void refreshPortAll() {
        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_PORTALL, new DecimalType(1));
    ...
    /**
     * Refreshes the front panel IR status. No corresponding protocol - simply return {@link OnOffType#ON}
     */
    void refreshIrEnable() {
        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_IRENABLE, OnOffType.ON);
    ...
    /**
     * Refreshes the saved preset number. No protocol to handle - simply return 0 (none)
     */
    void refreshSaveIo() {
        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_SAVEIO, new DecimalType(0));
    }
    ...
    /**
     * Refreshes the recall preset number. No protocol to handle - simply return 0 (none)
     */
    void refreshRecallIo() {
        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_RECALLIO, new DecimalType(0));
    }
    ...
    /**
     * Refreshes the clear preset number. No protocol to handle - simply return 0 (none)
     */
    void refreshClearIo() {
        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_CLEARIO, new DecimalType(0));
    }
    ...
    /**
     * Refreshes the matrix reset. No protocol to handle - simply return {@link OnOffType#OFF}
     */
    void refreshResetMatrix() {
        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_RESETMATRIX, OnOffType.OFF);
    }

    // void sendRS232Command(int zoneNbr, String rs232Cmd) {
    // String cmd = String.format(CMD_RS232_FORMAT, zoneNbr, rs232Cmd);
    // sendCommand(cmd, _rs232Pattern, new ResponseCallback() {
    // @Override
    // void handleResponse(Matcher m, String response) {
    // handleRs232Response(m, response);
    // }
    // });
    // }
    ...
    private void handlePowerResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 1) {
            switch (m.group(1)) {
                case "ON":
                    _callback.stateChanged(AtlonaPro3Constants.CHANNEL_POWER, OnOffType.ON);
                    break;
                case "OFF":
                    _callback.stateChanged(AtlonaPro3Constants.CHANNEL_POWER, OnOffType.OFF);
                    break;
                default:
                    logger.error("Invalid power response: '{}'", resp);
            }
        } else {
            logger.error("Invalid format power response: '{}'", resp);
        }
    }
    ...
    private void handleVersionResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 1) {
            ...
            logger.error("Invalid version response: '{}'", resp);
        }
    }
    ...
    private void handleTypeResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 1) {
            ...
            logger.error("Invalid Type response: '{}'", resp);
        }
    }
    ...
    private void handlePanelLockResponse(String resp) {
        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_PANELLOCK,
    ...
    private void handlePortPowerResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 2) {
            try {
                ...
                switch (m.group(2)) {
                    case "on":
                        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_PORTPOWER + portNbr, OnOffType.ON);
                        break;
                    case "off":
                        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_PORTPOWER + portNbr, OnOffType.OFF);
                        break;
                    default:
                        logger.error("Invalid port power response: '{}'", resp);
                }
            } catch (NumberFormatException e) {
                logger.error("Invalid port power (can't parse number): '{}'", resp);
            }
        } else {
            logger.error("Invalid port power response: '{}'", resp);
        }
    ...
    private void handlePortOutputResponse(Matcher m, String resp) {
        ...
        while (m.find()) {
            try {
                ...
                _callback.stateChanged(AtlonaPro3Constants.CHANNEL_PORTOUTPUT + outPort, new DecimalType(inPort));
            } catch (NumberFormatException e) {
                logger.error("Invalid switch response (can't parse number): '{}'", resp);
            }
        }
    }
    ...
    private void handleMirrorResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 2) {
            try {
                ...
                int outPortNbr = Integer.parseInt(m.group(2));
                _callback.stateChanged(AtlonaPro3Constants.CHANNEL_PORTMIRROR + hdmiPortNbr,
                        new DecimalType(outPortNbr));
            } catch (NumberFormatException e) {
    ...
    private void handleUnMirrorResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 1) {
            try {
                int hdmiPortNbr = Integer.parseInt(m.group(1));
                _callback.stateChanged(AtlonaPro3Constants.CHANNEL_PORTMIRROR + hdmiPortNbr, new DecimalType(0));
            } catch (NumberFormatException e) {
                logger.error("Invalid volume response (can't parse number): '{}'", resp);
            }
        } else {
    ...
    private void handleVolumeResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 2) {
            try {
                ...
                _callback.stateChanged(AtlonaPro3Constants.CHANNEL_VOLUME + portNbr, new DecimalType(level));
            } catch (NumberFormatException e) {
                logger.error("Invalid volume response (can't parse number): '{}'", resp);
            }
        } else {
    ...
    private void handleVolumeMuteResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 2) {
            try {
                ...
                switch (m.group(2)) {
                    case "on":
                        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_VOLUME_MUTE + portNbr, OnOffType.ON);
                        break;
                    case "off":
                        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_VOLUME_MUTE + portNbr, OnOffType.OFF);
                        break;
                    default:
                        logger.error("Invalid port power response: '{}'", resp);
                }
    ...
    private void handleIrLockResponse(String resp) {
        _callback.stateChanged(AtlonaPro3Constants.CHANNEL_IRENABLE,
    ...
    private void handleSaveIoResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 1) {
            try {
                int presetNbr = Integer.parseInt(m.group(1));
                _callback.stateChanged(AtlonaPro3Constants.CHANNEL_SAVEIO, new DecimalType(presetNbr));
            } catch (NumberFormatException e) {
                logger.error("Invalid volume response (can't parse number): '{}'", resp);
            }
        } else {
    ...
    private void handleRecallIoResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 1) {
            try {
                int presetNbr = Integer.parseInt(m.group(1));
                _callback.stateChanged(AtlonaPro3Constants.CHANNEL_RECALLIO, new DecimalType(presetNbr));
                refreshAllPortStatuses();
            } catch (NumberFormatException e) {
                logger.error("Invalid volume response (can't parse number): '{}'", resp);
            }
    ...
    private void handleClearIoResponse(Matcher m, String resp) {
        ...
        if (m.groupCount() == 1) {
            try {
                int presetNbr = Integer.parseInt(m.group(1));
                _callback.stateChanged(AtlonaPro3Constants.CHANNEL_CLEARIO, new DecimalType(presetNbr));
            } catch (NumberFormatException e) {
                logger.error("Invalid volume response (can't parse number): '{}'", resp);
            }
        } else {
    ...
    // private void handleRs232Response(Matcher m, String resp) {
