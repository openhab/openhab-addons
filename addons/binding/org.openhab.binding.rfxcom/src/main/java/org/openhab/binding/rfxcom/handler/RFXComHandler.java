...
public class RFXComHandler extends BaseThingHandler implements DeviceMessageListener {
		...
		if (bridgeHandler != null) {
			...
			try {
				...
				if (supportedValueSelectors.contains(valSelector)) {
					...
					logger.warn(
							"RFXCOM doesn't support tranmitting for channel '{}'",
							channelUID.getId());
				}
	...
	public void initialize() {
		...
		if (bridgeHandler == null) {
			// BridgeHandler is not yet initialized, start timer to check bridge
			// status ones a while
	...
	private void bridgeReadyWatchdog() {
		Runnable runnable = new Runnable() {
			public void run() {
				try {
					logger.trace("Check if bridge is initialized");
					getBridgeHandler();
				} catch (Exception e) {
		...
		if (bridgeHandler == null) {
			logger.debug("Brige isn't yet initialized, check state after {}sec", refresh);
	...
	private synchronized RFXComBridgeHandler getBridgeHandler() {
		...
		if (this.bridgeHandler == null) {
			...
			if (handler instanceof RFXComBridgeHandler) {
				this.bridgeHandler = (RFXComBridgeHandler) handler;
				logger.debug("Registering device message listener");
				this.bridgeHandler.registerDeviceStatusListener(this);
			} else {
				logger.debug("No available bridge handler found for {} bridge {} .", getThing().getUID().getThingTypeId(),
						bridge.getUID());
				return null;
			}
		} else {
			logger.trace("Bridge is initialized");
		}
