        ...
        try {
            ...
            if (connector instanceof RFXComJD2XXConnector) {
                if ("device not opened (3)".equalsIgnoreCase(e.getMessage())) {
                    logger.info("Automatic discovery of RFXCOM bridges using the FTDI chip driver (D2XX)"
                            + " do not work with OS native FTDI drivers loaded."
                            + " Either uninstall OS native FTDI drivers to allow D2XX"
                            + " or manually add a universal bridge 'RFXCOM USB Transceiver'"
                            + " and specify the serial port to avoid D2XX entirely.");
                }
            }
        } catch (Exception e) {
            logger.error("Connection to RFXCOM transceiver failed", e);
    ...
    private class MessageListener implements RFXComEventListener {
        ...
        public void packetReceived(byte[] packet) {
            try {
                ...
                logger.error("Communication error, data: {}", DatatypeConverter.printHexBinary(packet), e);
            } catch (IOException e) {
