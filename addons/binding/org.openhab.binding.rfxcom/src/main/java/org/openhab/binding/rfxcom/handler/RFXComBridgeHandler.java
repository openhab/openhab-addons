...
public class RFXComBridgeHandler extends BaseBridgeHandler {
		...
		try {
			...
			if (connector != null) {
				...
				if (configuration.ignoreConfig) {
					logger.info("Ignoring tranceiver configuration");
				} else {
					...
					try {
						...
						if (configuration.setMode != null
							...
							try {
								...
								logger.warn("setMode hexBinary value lenght should be 14 bytes (28 characters)");
							}
						}
					} finally {
	...
	public synchronized void sendMessage(RFXComMessage msg) {
			...
			try {
				...
				if (message instanceof RFXComInterfaceMessage) {
					...
					if (resp.seqNbr == seqNbr) {
						...
						logger.warn(
								"Sequnce number '{}' does not match, expecting number '{}'",
								resp.seqNbr, seqNbr);
					}
