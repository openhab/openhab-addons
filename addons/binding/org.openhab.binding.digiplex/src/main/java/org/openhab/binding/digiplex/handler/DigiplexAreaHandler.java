...
import org.openhab.binding.digiplex.communication.AreaArmDisarmResponse;
import org.openhab.binding.digiplex.communication.AreaArmRequest;
import org.openhab.binding.digiplex.communication.AreaDisarmRequest;
import org.openhab.binding.digiplex.communication.AreaQuickArmRequest;
import org.openhab.binding.digiplex.communication.AreaStatus;
import org.openhab.binding.digiplex.communication.AreaStatusRequest;
import org.openhab.binding.digiplex.communication.AreaStatusResponse;
import org.openhab.binding.digiplex.communication.ArmType;
...
import org.openhab.binding.digiplex.communication.events.AreaEvent;
...
public class DigiplexAreaHandler extends BaseThingHandler {
    ...
    @Nullable
    private DigiplexAreaMessageHandler visitor;
    private int areaNo;
    ...
    public void initialize() {
        ...
        visitor = new DigiplexAreaMessageHandler();
    ...
    private class DigiplexAreaMessageHandler implements DigiplexMessageHandler {
        ...
        public void handleAreaStatusResponse(AreaStatusResponse response) {
            if (response.isSuccess() && response.getAreaNo() == DigiplexAreaHandler.this.areaNo) {
                status = new StringType(response.getStatus().toString());
                armed = response.getStatus().toOpenClosedType();
                zoneInMemory = openClosedFromBoolean(response.isZoneInMemory());
                trouble = openClosedFromBoolean(response.isTrouble());
                ready = openClosedFromBoolean(response.isReady());
                inProgramming = openClosedFromBoolean(response.isInProgramming());
                alarm = openClosedFromBoolean(response.isAlarm());
                strobe = openClosedFromBoolean(response.isStrobe());
        ...
        public void handleArmDisarmAreaResponse(@NonNull AreaArmDisarmResponse response) {
            if (response.getAreaNo() == DigiplexAreaHandler.this.areaNo) {
