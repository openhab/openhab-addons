 ...
 * @since 1.1.0
 */
public abstract class Ihc2HttpsClient {

    private final Logger logger = LoggerFactory.getLogger(Ihc2HttpsClient.class);
     ...
     *            Url to connect.
     */
    protected void openConnection(String url) throws Ihc2Execption {
        if (client == null) {
            client = Ihc2ConnectionPool.getInstance().getHttpClient();
        }
     ...
     */
    protected String sendQuery(String query, int timeout) throws Ihc2Execption {
        try {
    ...
    private String sendQ(String query, int timeout)
        ...
        // Execute POST
        HttpResponse response = client.execute(postReq, Ihc2ConnectionPool.getInstance().getHttpContext());
