...
import static org.openhab.binding.ihc2.Ihc2BindingConstants.CHANNEL_DATETIME;
...
/**
 * The {@link Ihc2DateTimeThingHandler} is responsible for handling commands, which are
 * sent to one of the channels.
 *
 * @author Niels Peter Enemark - Initial contribution
 */
@NonNullByDefault
public class Ihc2DateTimeThingHandler extends BaseThingHandler implements Ihc2EventListener {

    private final Logger logger = LoggerFactory.getLogger(Ihc2DateTimeThingHandler.class);
    ...
    public Ihc2DateTimeThingHandler(Thing thing) {
        super(thing);
        logger.debug("Ihc2DateTimeThingHandler() for: {}", thing.getUID());
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        if (command == REFRESH) {
            try {
                Command cmd = Ihc2TypeUtils.ihc2oh(CHANNEL_DATETIME, ihcClient.resourceQuery(config.getResourceId()));
                postCommand(CHANNEL_DATETIME, cmd);
    ...
    public void initialize() {
        ...
        for (Channel c : channelList) {
            ihcClient.setStateDecription(c.getUID(),
                    new StateDescription(null, null, null, config.getPattern(), config.isReadonly(), null));
        }
    ...
    public void resourceValueUpdateReceived(@Nullable EventObject event, @Nullable WSResourceValue value) {
        ...
        try {
            Command cmd = Ihc2TypeUtils.ihc2oh(CHANNEL_DATETIME, value);
            postCommand(CHANNEL_DATETIME, cmd);
        } catch (Ihc2Execption e) {
