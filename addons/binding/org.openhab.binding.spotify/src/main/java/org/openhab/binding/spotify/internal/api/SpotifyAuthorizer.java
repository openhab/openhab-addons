...
public class SpotifyAuthorizer {
     ...
     */
    public AuthorizationCodeCredentials requestTokens(String redirectUri, String reqCode) {
     ...
     */
    public AuthorizationCodeCredentials refresh(String refreshToken) {
        logger.debug("Call Spotify to get a new refresh token.");
        return request(client -> postRequest(client, REFRESH_QUERY + refreshToken));
    }

    private AuthorizationCodeCredentials request(Function<HttpClient, Request> function) {
        ...
        return ModelUtil.gsonInstance().fromJson(response.getContentAsString(), AuthorizationCodeCredentials.class);
    }
