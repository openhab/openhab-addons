...
public class ApplicationCommandMessageClass extends ZWaveCommandProcessor {
    ...
    public boolean handleRequest(ZWaveController zController, SerialMessage lastSentMessage,
        ...
        if (zwaveCommandClass == null) {
            logger.debug("NODE {}: Command class {} not found, trying to add it.", nodeId, commandClass.getLabel(),
                    commandClass.getKey());
        ...
        if (zwaveCommandClass == null) {
            logger.error(String.format("NODE %d: Unsupported command class %s (0x%02x)", nodeId,
