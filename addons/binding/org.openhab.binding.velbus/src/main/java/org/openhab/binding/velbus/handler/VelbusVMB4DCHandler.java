...
/**
 * The {@link VelbusVMB4DCHandler} is responsible for handling commands, which are
 * sent to one of the channels.
 *
 * @author Cedric Boon - Initial contribution
 */
public class VelbusVMB4DCHandler extends VelbusThingHandler {
    public static final Set<ThingTypeUID> SUPPORTED_THING_TYPES = new HashSet<>(Arrays.asList(THING_TYPE_VMB4DC));

    public VelbusVMB4DCHandler(Thing thing) {
        super(thing, 4, 0, "Dimmer");
    }
    ...
    public void onPacketReceived(byte[] packet) {
        ...
        if (packet[0] == VelbusPacket.STX && packet.length >= 5) {
            ...
            if (command == COMMAND_DIMMERCONTROLLER_STATUS && packet.length >= 7) {
