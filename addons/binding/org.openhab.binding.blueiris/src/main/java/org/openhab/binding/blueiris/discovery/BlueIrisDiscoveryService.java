...
public class BlueIrisDiscoveryService extends AbstractDiscoveryService implements BridgeListener {
    ...
    public void onCamList(CamListReply camListReply) {
        ...
        for (CamListReply.Data data : camListReply.getCameras()) {
            String optionValue = data.getOptionValue();
            optionValue = optionValue.replaceAll("[^A-Za-z0-9_-]", "");
            ThingUID cameraUID = new ThingUID(BlueIrisBindingConstants.THING_TYPE_CAMERA, bridgeUid, optionValue);
            ...
            logger.info("Found camera {} -- {}", data.getOptionDisplay(), optionValue);
        }
    }
