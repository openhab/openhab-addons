 ...
 */
public class MotionSensorDataLightLevelReplyHandler extends MessageHandler {
    private static final Logger logger = LoggerFactory.getLogger(MotionSensorDataLightLevelReplyHandler.class);

    MotionSensorDataLightLevelReplyHandler(DeviceFeature p) {
        super(p);
    }
        ...
        switch (cmd2) {
            case 0x00: // this is a product data response message
                ...
                logger.debug("{}: {} got light level: {}, battery level: {}", nm(), handler.getAddress(), lightLevel,
                        batteryLevel);
                handler.updateFeatureState(f, new DecimalType(lightLevel));
                break;
            default:
