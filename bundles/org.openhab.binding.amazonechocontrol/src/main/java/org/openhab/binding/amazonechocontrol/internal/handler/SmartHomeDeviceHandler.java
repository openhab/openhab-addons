...
import static org.openhab.binding.amazonechocontrol.internal.AmazonEchoControlBindingConstants.CHANNEL_CONTROLLER_POWER_STATE;
import static org.openhab.binding.amazonechocontrol.internal.AmazonEchoControlBindingConstants.CHANNEL_CONTROLLER_SECURITY_PANEL_ARM_STATE;
...
public class SmartHomeDeviceHandler extends BaseThingHandler {
    ...
    public boolean setDeviceAndUpdateThingState(AccountHandler accountHandler,
        ...
        try {
            if (capabilities.contains(INTERFACE_POWER)) {
                addChannelToDevice(new ChannelUID(thing.getUID(), CHANNEL_CONTROLLER_POWER_STATE), ITEM_TYPE_SWITCH,
                        new ChannelTypeUID(BINDING_ID, CHANNEL_CONTROLLER_POWER_STATE));
            }
            ...
            if (capabilities.contains(INTERFACE_SECURITY_PANEL)) {
                addChannelToDevice(new ChannelUID(thing.getUID(), CHANNEL_CONTROLLER_SECURITY_PANEL_ARM_STATE),
                        ITEM_TYPE_STRING, new ChannelTypeUID(BINDING_ID, CHANNEL_CONTROLLER_SECURITY_PANEL_ARM_STATE));
            }
    ...
    public void updateState(List<SmartHomeBaseDevice> allDevices,
        ...
        for (SmartHomeDevice shd : GetSupportedSmartHomeDevices(smartHomeBaseDevice, allDevices)) {
            ...
            for (int i = 0; i < states.size(); i++) {
                ...
                if (stateJson.startsWith("{") && stateJson.endsWith("}")) {
                    ...
                    if (INTERFACE_POWER.equals(interfaceName)) {
                        if ("powerState".equals(state.get("name").getAsString())) {
                        ...
                        if ("brightness".equals(state.get("name").getAsString())) {
                        ...
                        if ("color".equals(state.get("name").getAsString())) {
                        ...
                        Set<String> capabilities = GetCapabilities(accountHandler, smartHomeBaseDevice);
                        if ("colorProperties".equals(state.get("name").getAsString())) {
                            if (capabilities.contains(INTERFACE_COLOR_TEMPERATURE)) {
                                // For groups take the first
                                if (colorTemperaturName == null) {
                                    colorTemperaturName = state.get("value").getAsJsonObject().get("name")
                                            .getAsString();
                                }
                    ...
                    } else if (INTERFACE_SECURITY_PANEL.equals(interfaceName)) {
                        if ("armState".equals(state.get("name").getAsString())) {
                            // For groups take the first
                            if (armState == null) {
            ...
            if (this.isLinked(CHANNEL_CONTROLLER_POWER_STATE)) {
                updateState(CHANNEL_CONTROLLER_POWER_STATE,
            ...
            if (this.isLinked(CHANNEL_CONTROLLER_SECURITY_PANEL_ARM_STATE)) {
                updateState(CHANNEL_CONTROLLER_SECURITY_PANEL_ARM_STATE,
                        armState == null ? UnDefType.UNDEF : new StringType(armState));
            }
            if (stateFound) {
                updateStatus(ThingStatus.ONLINE);
            } else {
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        try {
            ...
            for (SmartHomeDevice shd : devices) {
                ...
                if (channelId.equals(CHANNEL_CONTROLLER_POWER_STATE)
                ...
                if (channelId.equals(CHANNEL_CONTROLLER_SECURITY_PANEL_ARM_STATE)
                        && GetCapabilities(accountHandler, shd).contains(INTERFACE_SECURITY_PANEL)) {
                    if (command instanceof StringType) {
                        String armState = ((StringType) command).toFullString();
                        if (StringUtils.isNotEmpty(armState)) {
                            accountHandler.forceDelayedSmartHomeStateUpdate();
                            connection.smartHomeCommand(entityId, "controlSecurityPanel", "armState", armState);
                        }
                    }
                }
            }
    ...
    public boolean initialize(AccountHandler handler) {
        updateState(CHANNEL_CONTROLLER_POWER_STATE, OnOffType.OFF);
