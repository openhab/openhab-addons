...
class WebSocketConnectionImpl implements WebSocketConnection {
     ...
     */
    WebSocketConnectionImpl(ConsumedThing webThing, URI webSocketURI, ConnectionListener connectionListener, Duration pingPeriod) {
    ...
    private void close(String reason) {
        try {
            if (isOpen.getAndSet(false)) {
                logger.debug("websocket connection {} of {} closed. {}", webSocketURI, webThing.getThingDescription().title, reason);
    ...
    private final class WebSocketListener implements WebSocket.Listener {
        ...
        public CompletionStage<?> onText(WebSocket webSocket, CharSequence event, boolean last) {
            try {
                ...
                var message = receivedTextbufferRef.accumulateAndGet(event.toString(), (received, current) -> received + current);
                if (last) {
                    ...
                    if (isOpen.get()) {
                        if (propertyStatus.messageType.equals("propertyStatus")) {
                            for (var propertyName : propertyStatus.data.keySet()) {
                                propertyChangedListeners.getOrDefault(propertyName, EMPTY_PROPERTY_CHANGED_LISTENER).onPropertyValueChanged(webThing, propertyName, propertyStatus.data.get(propertyName));
                            }
                        } else {
        ...
        public void run() {
            ...
            while (isOpen.get()) {
                ...
                if (elapsedSinceLast.getSeconds() > pingPeriod.getSeconds()) {
                    close("connection seems to be broken (last message received at " + lastTimeReceived.get() + ", " + elapsedSinceLast.getSeconds() + " sec ago)");
                    return;
                }
            ...
            logger.debug("websocket of {} ({}) has been closed. terminating watchdog", webThing.getThingDescription().title, webSocketURI);
       }
