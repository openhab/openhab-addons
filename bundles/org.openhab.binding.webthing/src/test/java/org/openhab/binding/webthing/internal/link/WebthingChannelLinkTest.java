...
public class WebthingChannelLinkTest {
    ...
    public void testChannelToProperty() throws Exception {
        ...
        var channel = Channels.createChannel(thingUID, "target_position", webthing.getPropertyDescription("target_position"));
    ...
    public void testPropertyToChannel() throws Exception {
        ...
        var webthing = WebthingTest.createTestWebthing("http://example.org:8090/0", httpClientMock, websocketConnectionFactory);
        var channel = Channels.createChannel(thingUID, "target_position", webthing.getPropertyDescription("target_position"));

        var testWebthingThingHandler = new TestWebthingThingHandler();
        PropertyToChannelLink.establish(webthing, "target_position", testWebthingThingHandler, channel);

        websocketConnectionFactory.listeners.get("target_position").onPropertyValueChanged(webthing,"target_position", 70);
    ...
    public void testDataTypeMapping() throws Exception {
        ...
        performDataTypeMappingTest("color_prop", "#f2fe00", new HSBType("62.834644,100.0,99.60784"), "#ff0000", new HSBType("0.0,100.0,100.0"));
        performDataTypeMappingTest("colormode_prop", "color", new StringType("color"), "temperature", new StringType("temperature"));
        ...
        performDataTypeMappingTest("string_prop", "initial", new StringType("initial"), "updated", new StringType("updated"));
    ...
    private void performDataTypeMappingTest(String propertyName, Object initialValue, State initialState, Object updatedValue, State updatedState) throws Exception {
        HttpClientMock httpClientMock = new HttpClientMock();
        httpClientMock.onGet("http://example.org:8090/").doReturn(load("/datatypes_test_response.json"));
        httpClientMock.onGet("http://example.org:8090/properties/" + propertyName).doReturn(new Gson().toJson(Map.of(propertyName, initialValue)));
        var json = new Gson().toJson(Map.of(propertyName, updatedValue));
        httpClientMock.onPut("http://example.org:8090/properties/" + propertyName).withBody(is(json)).doReturnStatus(200);

        var thingUID = new ThingUID("webthing","test");
        var channelUID  = Channels.createChannelUID(thingUID, propertyName);

        var websocketConnectionFactory = new WebthingTest.TestWebsocketConnectionFactory();
        var webthing = WebthingTest.createTestWebthing("http://example.org:8090/", httpClientMock, websocketConnectionFactory);
        ...
        websocketConnectionFactory.listeners.get(propertyName).onPropertyValueChanged(webthing,propertyName, initialValue);
