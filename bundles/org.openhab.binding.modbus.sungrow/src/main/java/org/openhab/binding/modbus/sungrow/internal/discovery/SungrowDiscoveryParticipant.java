...
import static org.openhab.binding.modbus.sungrow.internal.SungrowConstants.SUPPORTED_THING_TYPES_UIDS;
...
@Component
@NonNullByDefault
public class SungrowDiscoveryParticipant implements ModbusDiscoveryParticipant {

    private final Logger logger = LoggerFactory.getLogger(SungrowDiscoveryParticipant.class);
    ...
    public void startDiscovery(ModbusEndpointThingHandler handler, ModbusDiscoveryListener listener) {
        logger.trace("Starting sungrow discovery");
        try {
            new SungrowDiscoveryProcess(handler, listener).detectModel();
