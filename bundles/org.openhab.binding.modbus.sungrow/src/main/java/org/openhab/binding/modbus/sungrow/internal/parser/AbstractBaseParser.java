...
public class AbstractBaseParser {
     ...
     * @return the parsed value or empty if the field is not implemented
     */
    protected Optional<Short> extractOptionalSInt16(ModbusRegisterArray raw, int index) {
        return ModbusBitUtilities.extractStateFromRegisters(raw, index, ValueType.INT16).map(DecimalType::shortValue)
                .filter(value -> value != (short) 0x8000);
    }
     ...
     * @return the parsed value or the default if the field is not implemented
     */
    protected Short extractSInt16(ModbusRegisterArray raw, int index, short def) {
        return extractOptionalSInt16(raw, index).orElse(def);
    }
    ...
    /**
     * Extract an optional uint32swap value
     *
     * @param raw the register array to extract from
     * @param index the address of the field
     * @return the parsed value or empty if the field is not implemented
     */
    protected Optional<Long> extractOptionalUint32Swap(ModbusRegisterArray raw, int index) {
        return ModbusBitUtilities.extractStateFromRegisters(raw, index, ValueType.UINT32_SWAP)
                .map(DecimalType::longValue).filter(value -> value != 0);
    }

    /**
     * Extract a mandatory uint32swap value
     *
     * @param raw the register array to extract from
     * @param index the address of the field
     * @param def the default value
     * @return the parsed value or default if the field is not implemented
     */
    protected Long extractUInt32Swap(ModbusRegisterArray raw, int index, long def) {
        return extractOptionalUint32Swap(raw, index).orElse(def);
    }

    /**
     * Extract an optional int32 value
     *
     * @param raw the register array to extract from
     * @param index the address of the field
     * @return the parsed value or empty if the field is not implemented
     */
    protected Optional<Long> extractOptionalSint32(ModbusRegisterArray raw, int index) {
        return ModbusBitUtilities.extractStateFromRegisters(raw, index, ValueType.INT32).map(DecimalType::longValue)
                .filter(value -> value != 0);
    }

    /**
     * Extract a mandatory sint32 value
     *
     * @param raw the register array to extract from
     * @param index the address of the field
     * @param def the default value
     * @return the parsed value or default if the field is not implemented
     */
    protected Long extractSInt32(ModbusRegisterArray raw, int index, long def) {
        return extractOptionalSint32(raw, index).orElse(def);
    }

    /**
     * Extract an optional int32swap value
     *
     * @param raw the register array to extract from
     * @param index the address of the field
     * @return the parsed value or empty if the field is not implemented
     */
    protected Optional<Long> extractOptionalSint32Swap(ModbusRegisterArray raw, int index) {
     ...
     * @return the parsed value or default if the field is not implemented
     */
    protected Long extractSInt32Swap(ModbusRegisterArray raw, int index, long def) {
        return extractOptionalSint32Swap(raw, index).orElse(def);
    }
