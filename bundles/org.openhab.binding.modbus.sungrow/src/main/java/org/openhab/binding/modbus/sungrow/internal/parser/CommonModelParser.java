...
import org.openhab.binding.modbus.sungrow.internal.SungrowConstants;
 ...
 */
@NonNullByDefault
public class CommonModelParser extends AbstractBaseParser implements SungrowParser<CommonModelBlock> {
    ...
    public CommonModelBlock parse(ModbusRegisterArray raw) {
        ...
        block.sungrowDID = extractUInt16(raw, 0, 0);

        block.length = extractUInt16(raw, 1, raw.size() - SungrowConstants.MODEL_HEADER_SIZE);

        if (block.length + SungrowConstants.MODEL_HEADER_SIZE != raw.size()) {
            logger.warn("Short read on common block loding. Expected size: {}, got: {}",
                    block.length + SungrowConstants.MODEL_HEADER_SIZE, raw.size());
            return block;
        }
