 ...
 */
@NonNullByDefault
public class InverterModelParser extends AbstractBaseParser implements SungrowParser<InverterModelBlock> {

    @Override
    public InverterModelBlock parse(ModbusRegisterArray raw) {
        InverterModelBlock block = new InverterModelBlock();

        // block.phaseConfiguration = extractUInt16(raw, 0, SungrowConstants.INVERTER_SHRT);
        ...
        block.mppt1Current = extractUInt16(raw, 27, 0);
        block.mppt1Voltage = extractUInt16(raw, 29, 0);
        block.mppt1Power = block.mppt1Voltage * block.mppt1Current;
        block.mppt2Current = extractUInt16(raw, 29, 0);
        block.mppt2Voltage = extractUInt16(raw, 30, 0);
        block.mppt2Power = block.mppt2Voltage * block.mppt2Current;
        block.mpptCurrentSF = -1;
        block.mpptVoltageSF = -1;
        block.mpptPowerSF = -2;

        block.temperatureCabinet = extractInt16(raw, 33, (short) 0);
        block.temperatureSF = -1;
