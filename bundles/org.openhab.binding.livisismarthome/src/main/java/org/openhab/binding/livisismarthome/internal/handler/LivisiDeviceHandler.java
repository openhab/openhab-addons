...
/**
 * The {@link LivisiDeviceHandler} is responsible for handling the {@link Device}s and their commands, which are
...
public class LivisiDeviceHandler extends BaseThingHandler implements DeviceStatusListener {
    ...
    public void handleCommand(final ChannelUID channelUID, final Command command) {
        ...
        @Nullable
        final LivisiBridgeHandler livisiBridgeHandler = getBridgeHandler();
        if (livisiBridgeHandler == null) {
            logger.warn("BridgeHandler not found. Cannot handle command without bridge.");
            return;
        }
        if (!ThingStatus.ONLINE.equals(livisiBridgeHandler.getThing().getStatus())) {
        ...
        if (command instanceof RefreshType) {
            @Nullable
            final Device device = livisiBridgeHandler.getDeviceById(deviceId);
            if (device != null) {
        ...
        if (CHANNEL_SWITCH.equals(channelUID.getId())) {
            ...
            if (command instanceof OnOffType) {
                livisiBridgeHandler.commandSwitchDevice(deviceId, OnOffType.ON.equals(command));
            }
            ...
            if (command instanceof DecimalType) {
                ...
                livisiBridgeHandler.commandSetDimmLevel(deviceId, dimLevel.intValue());
            } else if (command instanceof OnOffType) {
                if (OnOffType.ON.equals(command)) {
                    livisiBridgeHandler.commandSetDimmLevel(deviceId, 100);
                } else {
                    livisiBridgeHandler.commandSetDimmLevel(deviceId, 0);
                }
            }
            ...
            if (command instanceof DecimalType) {
                ...
                livisiBridgeHandler.commandSetRollerShutterLevel(deviceId,
                        invertValueIfConfigured(CHANNEL_ROLLERSHUTTER, rollerShutterLevel.intValue()));
            } else if (command instanceof OnOffType) {
                if (OnOffType.ON.equals(command)) {
                    livisiBridgeHandler.commandSetRollerShutterStop(deviceId, ShutterAction.ShutterActions.DOWN);
                } else {
                    livisiBridgeHandler.commandSetRollerShutterStop(deviceId, ShutterAction.ShutterActions.UP);
                }
                ...
                if (StopMoveType.STOP.equals(command)) {
                    livisiBridgeHandler.commandSetRollerShutterStop(deviceId, ShutterAction.ShutterActions.STOP);
                }
            }
            ...
            if (command instanceof DecimalType) {
                ...
                livisiBridgeHandler.commandUpdatePointTemperature(deviceId, pointTemperature.doubleValue());
            }
            ...
            if (command instanceof StringType) {
                ...
                if (CapabilityState.STATE_VALUE_OPERATION_MODE_AUTO.equals(autoModeCommand)) {
                    livisiBridgeHandler.commandSetOperationMode(deviceId, true);
                } else if (CapabilityState.STATE_VALUE_OPERATION_MODE_MANUAL.equals(autoModeCommand)) {
                    livisiBridgeHandler.commandSetOperationMode(deviceId, false);
                } else {
            ...
            if (command instanceof OnOffType) {
                livisiBridgeHandler.commandSwitchAlarm(deviceId, OnOffType.ON.equals(command));
            }
        } else {
    ...
    /**
     * Returns the {@link Device} associated with this {@link LivisiDeviceHandler} (referenced by the
     * {@link LivisiDeviceHandler#deviceId}).
     *
     * @return the {@link Device} or null, if not found or no {@link LivisiBridgeHandler} is available
     */
    private @Nullable Device getDevice() {
        if (getBridgeHandler() != null) {
            return getBridgeHandler().getDeviceById(deviceId);
        }
        return null;
    }
     ...
     */
    private @Nullable LivisiBridgeHandler getBridgeHandler() {
        synchronized (this.lock) {
        ...
        synchronized (this.lock) {
            ...
            if (event.isLinkedtoCapability()) {
                ...
                if (capability.hasState()) {
                    ...
                    final LivisiBridgeHandler livisiBridgeHandler = getBridgeHandler();
                    if (livisiBridgeHandler != null) {
                        device = livisiBridgeHandler.refreshDevice(deviceId);
                    }
                    if (device != null) {
