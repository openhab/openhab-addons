...
@NonNullByDefault
public class A5_3F_7F_EltakoFSB extends _4BSMessage implements StateMachineProvider<BlindAction, BlindState> {
    ...
    private void convertDimmerImpl(int newPos, int actPos, Configuration config,
            STMStateMachine<BlindAction, BlindState> stm) {
        ...
        if (duration != 0) {
            ...
            stm.apply(BlindAction.SLATS_POS_REQUEST);
            return;
        } else {
            stm.apply(BlindAction.SLATS_POS_DONE); // safe way to return to IDLE
        }
    }
            ...
            if ((stm.getState() == BlindState.IDLE) || (stm.getState() == BlindState.INVALID)) {
                if (upDownCommand == UpDownType.UP) {
                    ...
                    if (stm.getState() == BlindState.IDLE) {
                        ...
                        stm.apply(BlindAction.CALIBRATION_REQUEST_DOWN);
                    }
        ...
        if (stm != null) {
            if (stopMoveCommand == StopMoveType.STOP) {
                ...
                if (stm.getState() == BlindState.POSITION_REACHED) {
                    ...
                    PercentType currentDimmer = channelDimmer.as(PercentType.class);
                    if (currentDimmer != null) {
                        if (stm.getPrevState() == BlindState.MOVEMENT_POSITION_UP) {
        ...
        if (channel != null) {
            ...
            switch (channelId) {
                case CHANNEL_ROLLERSHUTTER:
                    processChannelRollershutter(thing, command, config, getCurrentStateFunc, blindStm);
                    break;
                case CHANNEL_DIMMER:
                    processChannelDimmer(thing, command, config, getCurrentStateFunc, blindStm);
                    break;
            }
        }
    }
            ...
            if (currentState != null) {
                ...
                if (duration == c.shutTime) {
                    if (blindStm != null) {
                        ...
                        blindStm.apply(BlindAction.CALIBRATION_DONE); // will launch new command for setting the
                                                                      // position
                    }
                    ...
                    if (current != null && c.shutTime != -1 && c.shutTime != 0) {
                        if (blindStm != null) {
                            switch (blindStm.getState()) {
                                case MOVEMENT_POSITION_UP, MOVEMENT_POSITION_DOWN:
                                    blindStm.storeCommand(CHANNEL_ROLLERSHUTTER, StopMoveType.MOVE);
                                    blindStm.apply(BlindAction.POSITION_DONE); // will execute MOVE command, when we use
                                    // blinds
                                ...
                                case MOVEMENT_SLATS:
                                    blindStm.apply(BlindAction.SLATS_POS_DONE);
    ...
    // StateMachineProvider interface implementation
    ...
    @Override
    public @Nullable STMTransitionConfiguration<BlindAction, BlindState> getTransitionConfiguration(Thing thing) {
        ...
        return switch (mode) {
            case LEGACY -> null;
            case ROLLERSHUTTER -> STMTransitionConfiguration.BlindTransitions.FSB14_ROLLERSHUTTER;
            case BLINDS -> STMTransitionConfiguration.BlindTransitions.FSB14_BLINDS;
        };
    }

    @Override
    public BlindState getInitialState() {
    ...
    public Set<BlindAction> getRequiredCallbackActions(Thing thing) {
        ...
        return switch (mode) {
            case LEGACY -> Set.of();
            case ROLLERSHUTTER -> Set.of(BlindAction.CALIBRATION_DONE);
            case BLINDS -> Set.of(BlindAction.CALIBRATION_DONE, BlindAction.POSITION_DONE);
        };
    }
