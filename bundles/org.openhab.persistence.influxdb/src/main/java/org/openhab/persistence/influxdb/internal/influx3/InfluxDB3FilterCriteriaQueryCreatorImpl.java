...
/**
 * Implementation of {@link FilterCriteriaQueryCreator} for InfluxDB 3.0
 *
 * InfluxDB 3 databases have no retention-policy concept, so unlike the InfluxDB 1.0 implementation this queries the
 * measurement directly instead of a retention-policy-qualified table name; the {@code retentionPolicy} parameter is
 * accepted only to satisfy the shared {@link FilterCriteriaQueryCreator} contract.
 *
 * @author Cedric Boon - Initial contribution
 */
@NonNullByDefault
public class InfluxDB3FilterCriteriaQueryCreatorImpl implements FilterCriteriaQueryCreator {

    private final InfluxDBConfiguration configuration;
    private final InfluxDBMetadataService influxDBMetadataService;

    public InfluxDB3FilterCriteriaQueryCreatorImpl(InfluxDBConfiguration configuration,
    ...
    public String createQuery(FilterCriteria criteria, String retentionPolicy, @Nullable String alias) {
        ...
        Select select = select().column("\"" + COLUMN_VALUE_NAME_V1 + "\"::field")
                .column("\"" + TAG_ITEM_NAME + "\"::tag").fromRaw(null, "\"" + tableName + "\"");

        Where where = select.where();

        if (!tableName.equals(localAlias)) {
