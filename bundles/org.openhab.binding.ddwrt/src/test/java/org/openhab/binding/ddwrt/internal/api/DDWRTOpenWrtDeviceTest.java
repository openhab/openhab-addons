...
class DDWRTOpenWrtDeviceTest {
    ...
    @SuppressWarnings("null")
    @Test
    void testLeavesRadioEnabledStateUnknownWhenInterfaceFlagsAreUnavailable() {
        SshRunner runner = mock(SshRunner.class);
        when(runner.execStdout("iwinfo")).thenReturn("""
                phy0-ap0 ESSID: "magickingdom"
                         Mode: Master  Channel: 6 (2.437 GHz)
                ...
                """);
        when(runner.execStdout("cat /sys/class/net/phy0-ap0/flags")).thenReturn("");
        when(runner.execStdout("cat /sys/class/net/phy1-ap0/flags")).thenReturn("invalid");
        when(runner.execStdout("iwinfo phy0-ap0 info | grep -i channel | head -1 | grep -oE '[0-9]+'")).thenReturn("");
        when(runner.execStdout("iwinfo phy1-ap0 info | grep -i channel | head -1 | grep -oE '[0-9]+'")).thenReturn("");

        List<DDWRTRadio> radios = IwinfoParser.enumerateRadios(mock(Logger.class), runner, DEVICE_MAC);

        assertThat(radios.size(), is(2));
        assertThat(radios.get(0).isEnabled(), nullValue());
        assertThat(radios.get(1).isEnabled(), nullValue());
    }
    ...
    void testControlsRadioThroughOpenWrtWirelessService() throws IOException {
        ...
        verify(runner, times(2)).execStdout(RADIO_LOOKUP_COMMAND);
    }
