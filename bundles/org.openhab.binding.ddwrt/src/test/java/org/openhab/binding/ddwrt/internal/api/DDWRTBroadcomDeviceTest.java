...
class DDWRTBroadcomDeviceTest {
    ...
    @Test
    void testEnumerateRadiosRefreshesVapsAndRetriesCachedPhysicalInterfaces() {
        SshRunner runner = mock(SshRunner.class);
        when(runner.execStdout(anyString())).thenReturn("");
        when(runner.execStdout("nvram get wl0_vifs; nvram get wl1_vifs; nvram get wl2_vifs")).thenReturn("",
                "wl0.1 wl0.2");
        ...
        List<DDWRTRadio> radiosAfterVapsAdded = device.enumerateRadios(runner);
        List<DDWRTRadio> radiosAfterPhysicalInterfaceRecovers = device.enumerateRadios(runner);

        assertThat(initialRadios.stream().map(DDWRTRadio::getIfaceName).toList(), contains("eth1", "eth2"));
        assertThat(radiosAfterVapsAdded.stream().map(DDWRTRadio::getIfaceName).toList(),
                contains("eth1", "wl0.1", "wl0.2"));
        assertThat(radiosAfterPhysicalInterfaceRecovers.stream().map(DDWRTRadio::getIfaceName).toList(),
