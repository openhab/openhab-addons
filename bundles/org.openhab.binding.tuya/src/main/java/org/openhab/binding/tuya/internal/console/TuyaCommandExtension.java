...
public class TuyaCommandExtension extends AbstractConsoleCommandExtension implements ConsoleCommandCompleter {
    ...
    public TuyaCommandExtension(final @Reference ThingRegistry thingRegistry,
            final @Reference TuyaSchemaService schemaService) {
    ...
    private void printSchema(String productId, Console console) {
        ...
        console.printf("%4s  %-32s %-8s %-10s %s%n", "DP", "Code", "Type", "Access", "Details");
        schema.values().forEach(schemaDp -> console.printf("%4d  %-32s %-8s %-10s %s%n", schemaDp.id, schemaDp.code,
                schemaDp.type, schemaDp.readOnly ? "read-only" : "read/write", details(schemaDp)));
    }
    ...
    private void reloadSchema(String thingUidString, Console console) {
        ...
        if (thing == null || !DEVICE_THING_TYPES.contains(thing.getThingTypeUID())) {
        ...
        try {
            schemaDps = schemaService.reloadSchema(configuration.productId, configuration.deviceId).get();
        } catch (ExecutionException e) {
            Throwable cause = e.getCause();
            console.println(
                    "Reloading the schema failed: " + (cause != null ? cause.getMessage() : e.getMessage()) + ".");
            return;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            return;
        }
        console.println("Stored schema with " + schemaDps.size() + " data points for product '"
                + configuration.productId + "'.");

        List<ThingUID> reinitialized = schemaService.reinitializeThings(configuration.productId);
        if (reinitialized.isEmpty()) {
            console.println("No enabled thing uses this product, the schema is used when a thing is enabled.");
        }
        reinitialized.forEach(uid -> console.println("Re-initialized thing '" + uid + "'."));
    }

    private Stream<Thing> deviceThings() {
        return thingRegistry.getAll().stream().filter(thing -> DEVICE_THING_TYPES.contains(thing.getThingTypeUID()));
    }
