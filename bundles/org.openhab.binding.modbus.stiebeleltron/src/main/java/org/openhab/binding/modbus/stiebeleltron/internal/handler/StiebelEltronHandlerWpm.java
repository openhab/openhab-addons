 ...
 * @author Thomas Burri - Thing handler for a WPM3 compatible heat pump
 */
@NonNullByDefault
public class StiebelEltronHandlerWpm extends BaseThingHandler {
    public abstract class AbstractBasePoller {

        private final Logger logger = LoggerFactory.getLogger(StiebelEltronHandlerWpm.class);
        ...
        public synchronized void unregisterPollTask() {
            ...
            ModbusCommunicationInterface mycomms = StiebelEltronHandlerWpm.this.comms;
            if (mycomms != null) {
        ...
        public synchronized void registerPollTask(int address, int length, ModbusReadFunctionCode readFunctionCode) {
            ...
            ModbusCommunicationInterface mycomms = StiebelEltronHandlerWpm.this.comms;
            StiebelEltronConfiguration myconfig = StiebelEltronHandlerWpm.this.config;
            ...
            }, StiebelEltronHandlerWpm.this::handleReadError);
        }
     ...
     */
    public StiebelEltronHandlerWpm(Thing thing) {
        super(thing);
    }
    ...
    protected void writeInt16(int address, short shortValue) {
        ...
        mycomms.submitOneTimeWrite(request, result -> {
            ...
            StiebelEltronHandlerWpm.this.updateStatus(ThingStatus.ONLINE);
        }, failure -> {
            StiebelEltronHandlerWpm.this.handleWriteError(failure);
        });
    }
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        if (RefreshType.REFRESH == command) {
            ...
            if (groupId != null) {
                ...
                switch (groupId) {
                    case GROUP_SYSTEM_INFORMATION_WPM:
                        poller = systemInformationPoller;
                        break;
                    case GROUP_SYSTEM_PARAMETER_WPM:
                        poller = systemParameterPoller;
                        break;
                    case GROUP_SYSTEM_STATE_WPM:
            ...
            try {
                if (GROUP_SYSTEM_PARAMETER_WPM.equals(channelUID.getGroupId())) {
    ...
    protected void handlePolledSystemInformationData(ModbusRegisterArray registers) {
        ...
        if (block.temperatureFe7 != -32768) {
            updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_FE7_TEMPERATURE),
                    getScaled(block.temperatureFe7, 10, CELSIUS));
            updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_FE7_TEMPERATURE_SETPOINT),
                    getScaled(block.temperatureFe7SetPoint, 10, CELSIUS));
        } else {
            logger.trace("FE7 not available");
        }

        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_OUTDOOR_TEMPERATURE),
                getScaled(block.temperatureOutdoor, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_HC1_TEMPERATURE),
                getScaled(block.temperatureHc1, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_HC1_TEMPERATURE_SETPOINT),
                getScaled(block.temperatureHc1SetPoint, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_HC2_TEMPERATURE),
                getScaled(block.temperatureHc2, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_HC2_TEMPERATURE_SETPOINT),
                getScaled(block.temperatureHc2SetPoint, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_HP_FLOW_TEMPERATURE),
                getScaled(block.temperatureFlowHp, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_NHZ_FLOW_TEMPERATURE),
                getScaled(block.temperatureFlowNhz, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_FLOW_TEMPERATURE),
        ...
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_HEATING_PRESSURE),
        ...
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_FAN_COOLING_TEMPERATURE),
                getScaled(block.temperatureFanCooling, 10, KELVIN));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_FAN_COOLING_TEMPERATURE_SETPOINT),
                getScaled(block.temperatureFanCoolingSetPoint, 10, KELVIN));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_AREA_COOLING_TEMPERATURE),
                getScaled(block.temperatureAreaCooling, 10, KELVIN));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_AREA_COOLING_TEMPERATURE_SETPOINT),
                getScaled(block.temperatureAreaCoolingSetPoint, 10, KELVIN));

        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_EXT_HEAT_SOURCE_TEMPERATURE),
                getScaled(block.temperatureExtHeatSource, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_EXT_HEAT_SOURCE_TEMPERATURE_SETPOINT),
        ...
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_SOURCE_TEMPERATURE),
                getScaled(block.temperatureSource, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_INFORMATION_WPM, CHANNEL_MIN_SOURCE_TEMPERATURE),
    ...
    protected void handlePolledSystemParameterData(ModbusRegisterArray registers) {
        ...
        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_OPERATING_MODE),
                new DecimalType(block.operationMode));

        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_HC1_COMFORT_TEMPERATURE),
                getScaled(block.comfortTemperatureHeating, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_HC1_ECO_TEMPERATURE),
        ...
        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_HC1_HEATING_CURVE_RISE),
                new DecimalType(heatingCurveRiseHeatingHk1));

        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_HC2_COMFORT_TEMPERATURE),
                getScaled(block.comfortTemperatureHeatingHc2, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_HC2_ECO_TEMPERATURE),
        ...
        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_HC2_HEATING_CURVE_RISE),
        ...
        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_FIXED_VALUE_OPERATION),
                getScaled((block.fixedValueOperation == -28672) ? 0 : block.fixedValueOperation, 10, CELSIUS));

        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_HOTWATER_COMFORT_TEMPERATURE),
                getScaled(block.comfortTemperatureWater, 10, CELSIUS));
        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_HOTWATER_ECO_TEMPERATURE),
        ...
        if (block.hotwaterStages != 32768) {
            ...
            updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_HOTWATER_STAGES),
        ...
        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_HOTWATER_DUAL_MODE_TEMPERATURE),
        ...
        if (block.flowTemperatureAreaCooling != -32768) {
            ...
            updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_AREA_COOLING_FLOW_TEMPERATURE_SETPOINT),
                    getScaled(block.flowTemperatureAreaCooling, 10, CELSIUS));
            updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_AREA_COOLING_ROOM_TEMPERATURE_SETPOINT),
        ...
        if (block.flowTemperatureFanCooling != -32768) {
            ...
            updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_FAN_COOLING_FLOW_TEMPERATURE_SETPOINT),
                    getScaled(block.flowTemperatureFanCooling, 10, CELSIUS));
            updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_FAN_COOLING_ROOM_TEMPERATURE_SETPOINT),
            ...
            if (block.flowTemperatureHysteresisFanCooling != -32768) {
                ...
                updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_FAN_COOLING_FLOW_TEMPERATURE_HYSTERESIS),
                        getScaled(block.flowTemperatureHysteresisFanCooling, 10, KELVIN));
            }
        }

        logger.trace("reset = {}", block.reset);
        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_RESET), new DecimalType(block.reset));
        logger.trace("restartISG = {}", block.restartIsg);
        updateState(channelUID(GROUP_SYSTEM_PARAMETER_WPM, CHANNEL_RESTART_ISG), new DecimalType(block.restartIsg));
    ...
    protected void handlePolledSystemStateData(ModbusRegisterArray registers) {
        ...
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_HC1_PUMP_ACTIVE),
                (block.state & 1) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_HC2_PUMP_ACTIVE),
                (block.state & 2) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_HEAT_UP_PROGRAM_ACTIVE),
                (block.state & 4) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_NHZ_RUNNING),
                (block.state & 8) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_HP_IN_HEATING_MODE),
                (block.state & 16) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_HP_IN_HOTWATER_MODE),
                (block.state & 32) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_COMPRESSOR_RUNNING),
                (block.state & 64) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_SUMMER_MODE_ACTIVE),
                (block.state & 128) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_COOLING_MODE_ACTIVE),
                (block.state & 256) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_MIN_ONE_IWS_IN_DEFROSTING_MODE),
                (block.state & 512) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);

        if (block.operatingStatus != 32768) {
            updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_SILENT_MODE1_ACTIVE),
                    (block.operatingStatus & 1024) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
            updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_SILENT_MODE2_ACTIVE),
                    (block.operatingStatus & 2048) != 0 ? OpenClosedType.OPEN : OpenClosedType.CLOSED);
        }

        // power-off bit-coded
        logger.trace("powerOff = {}", block.powerOff);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_POWER_OFF),
                (block.powerOff == 1) ? OpenClosedType.OPEN : OpenClosedType.CLOSED);

        logger.trace("faultStatus = {}", block.faultStatus);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_FAULT_STATUS), new DecimalType(block.faultStatus));

        logger.trace("busStatus = {}", block.busStatus);
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_BUS_STATUS), new DecimalType(block.busStatus));

        logger.trace("defrostInitiated = {}", block.defrostInitiated);
        if (block.defrostInitiated != 32768) {
            updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_DEFROST_INITIATED),
        ...
        updateState(channelUID(GROUP_SYSTEM_STATE_WPM, CHANNEL_ACTIVE_ERROR), new DecimalType(block.activeError));
