...
public class UniFiProtectPrivateClient {
     ...
     * {@code unifi:controller} bridge.
     *
     * @param httpClient The HTTP client to use (provided by the parent bridge handler)
     * @param scheduler The scheduled executor service to use for periodic tasks (also provided by the parent)
     * @param baseUrl The console base URL, e.g. {@code https://192.168.1.1:443}
     * @param session The authenticated session owned by the parent bridge handler
     */
    public UniFiProtectPrivateClient(HttpClient httpClient, ScheduledExecutorService scheduler, String baseUrl,
            UniFiSession session) {
        this.httpClient = httpClient;
        this.scheduler = scheduler;
        this.baseUrl = baseUrl;
        this.authenticator = new UniFiProtectAuthenticator(httpClient, scheduler, baseUrl, session);
    }
