...
import org.openhab.binding.insteon2.internal.device.DeviceType;
import org.openhab.binding.insteon2.internal.device.InsteonDevice;
import org.openhab.binding.insteon2.internal.device.InsteonEngine;
...
public abstract class InsteonBaseThingHandler extends BaseThingHandler implements InsteonThingHandler {
    ...
    private List<Channel> getCustomChannels(String channelTypeId) {
        return getThing().getChannels().stream().filter(channel -> {
            ChannelTypeUID channelTypeUID = channel.getChannelTypeUID();
            return channelTypeUID != null && channelTypeUID.getId().equals(channelTypeId)
                    && !channel.getUID().getId().equals(channelTypeId);
        }).toList();
    }
    ...
    public void updateProperties(Device device) {
        ...
        if (device instanceof InsteonDevice insteonDevice) {
            InsteonEngine engine = insteonDevice.getInsteonEngine();
