...
import static org.openhab.binding.unifi.internal.network.UniFiBindingConstants.DEVICE_TYPE_UAP;
import static org.openhab.binding.unifi.internal.network.UniFiBindingConstants.PARAMETER_CID;
import static org.openhab.binding.unifi.internal.network.UniFiBindingConstants.PARAMETER_MAC_ADDRESS;
import static org.openhab.binding.unifi.internal.network.UniFiBindingConstants.PARAMETER_NID;
import static org.openhab.binding.unifi.internal.network.UniFiBindingConstants.PARAMETER_PORT_NUMBER;
import static org.openhab.binding.unifi.internal.network.UniFiBindingConstants.PARAMETER_SID;
import static org.openhab.binding.unifi.internal.network.UniFiBindingConstants.PARAMETER_SITE;
import static org.openhab.binding.unifi.internal.network.UniFiBindingConstants.PARAMETER_WID;
...
import org.openhab.binding.unifi.internal.network.UniFiBindingConstants;
...
/**
 * Discovery service for detecting Network things behind a {@code unifi:controller} bridge. Uses the shared
 * authenticated session published by the parent binding to run a one-off refresh and populate discovered things.
 *
...
@NonNullByDefault
public class UniFiThingDiscoveryService extends AbstractThingHandlerDiscoveryService<UniFiControllerBridgeHandler> {
    ...
    public UniFiThingDiscoveryService() {
        super(UniFiControllerBridgeHandler.class, UniFiBindingConstants.THING_TYPE_SUPPORTED,
    ...
    protected void startScan() {
        ...
        try {
            session = thingHandler.getSessionAsync().get(10, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            return;
        } catch (TimeoutException e) {
            logger.debug("Discovery scan aborted: bridge session not yet available");
            return;
        } catch (Exception e) {
            logger.debug("Discovery scan aborted: bridge session not available: {}", e.getMessage());
            return;
        }

        Thing bridgeThing = thingHandler.getThing();
        ...
        int timeoutSeconds = timeoutObj instanceof Number n ? n.intValue() : 30;

        final UniFiController controller = new UniFiController(thingHandler.getHttpClient(), session, unifios,
                timeoutSeconds);
        try {
            controller.refresh();
            final UniFiControllerCache cache = controller.getCache();
            final ThingUID bridgeUID = bridgeThing.getUID();
