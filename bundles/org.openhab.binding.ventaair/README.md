...
It currently supports the LW60-T device as well as a generic `ThingType` for other models.
...
| fanSpeed       | Number | This is the channel to control the steps for the speed of the fan |
| targetHumidity | Number | This channel sets the target humidity that should be try to reach by the device  |
| timer          | Number | This channel sets the power off timer to the set value in hours, i.e. 3 = turn off in 3 hours from now |
...
| humidity       | Number (RO) | This channel provides the humidity measured by the device in percent |
...
| timerTimePassed  | Number (RO) | If a timer has been set, this channel provides the minutes since when the timer was started  |
| operationTime  | Number (RO) | This channel provides the operation time of the device in hours |
| discReplaceTime | Number (RO) | This channel provides the time in how many hours the cleaning disc should be replaced  |
| cleaningTime   | Number (RO) | This channel provides the time in how many hours the device should be cleaned  |
