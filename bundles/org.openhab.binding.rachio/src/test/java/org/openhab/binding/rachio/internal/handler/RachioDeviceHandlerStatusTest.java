...
class RachioDeviceHandlerStatusTest {
    ...
    void runningCurrentScheduleWithBlankNameUsesScheduleThingLabelFallback() throws Exception {
        Thing thing = thing();
        RachioDevice device = deviceWithZoneWithoutSchedules();
    ...
    private RachioBridgeHandler bridgeHandler(Thing thing, RachioDevice device) {
        ...
        try {
            when(bridgeHandler.getCurrentSchedule(eq(DEVICE_ID), any(RequestPurpose.class)))
                    .thenReturn(new RachioCurrentScheduleResponse());
            when(bridgeHandler.getDeviceForecast(DEVICE_ID, "US")).thenReturn(new RachioForecastResponse());
    ...
    private RachioDevice deviceWithZone() {
        return deviceWithZone(true);
    }

    private RachioDevice deviceWithZoneWithoutSchedules() {
        return deviceWithZone(false);
    }

    private RachioDevice deviceWithZone(boolean includeSchedule) {
        RachioCloudDevice cloudDevice = new RachioCloudDevice();
        cloudDevice.id = DEVICE_ID;
        cloudDevice.name = "Rachio-E0ACCA";
    ...
    private static RachioApiThrottledException throttledException() {
        return new RachioApiThrottledException(
                new RateLimitThrottleException(Priority.MEDIUM, RequestPurpose.INITIALIZATION, 0.1, 0.2),
                new RachioApiResult());
    }
