...
class RachioSmartHoseTimerHandlerTest {
    ...
    void modernValveRunStartEventDirectlyUpdatesValveThing() throws Exception {
        ...
        verify(fixture.bridgeHandler, Mockito.timeout(1000)).getValveDayViews(VALVE_ID);
    ...
    void modernValveRunEndEventDirectlyUpdatesValveThing() throws Exception {
        ...
        verify(fixture.bridgeHandler, Mockito.timeout(1000)).getValveDayViews(VALVE_ID);
        awaitWebhookSummaryRefresh(fixture.handler);
    ...
    private SmartHoseValveFixture smartHoseValveFixture(boolean hoseTimerWebhooksEnabled) throws Exception {
        ...
        handler.initialize();
        clearInvocations(callback, bridgeHandler);
        return new SmartHoseValveFixture(bridgeHandler, thing, callback, handler);
    }
    ...
    private void configureHoseTimerWebhookMode(RachioBridgeHandler bridgeHandler, boolean hoseTimerWebhooksEnabled)
        ...
        RachioWebhookMode mode = hoseTimerWebhooksEnabled ? RachioWebhookMode.WEBHOOK_SERVICE
                : RachioWebhookMode.DISABLED;
        Mockito.doReturn(mode).when(bridgeHandler).getWebhookMode(RachioWebhookResourceType.VALVE);
        Mockito.doReturn(mode).when(bridgeHandler).getWebhookMode(RachioWebhookResourceType.PROGRAM);
    }
    ...
    private void awaitWebhookSummaryRefresh(TestValveHandler handler) throws Exception {
        Field field = findField(handler, "webhookSummaryRefreshPending");
        field.setAccessible(true);
        AtomicBoolean pending = (AtomicBoolean) field.get(handler);
    ...
    private Field findField(Object target, String fieldName) throws NoSuchFieldException {
    ...
    private record SmartHoseValveFixture(RachioBridgeHandler bridgeHandler, Thing thing, ThingHandlerCallback callback,
            TestValveHandler handler) {
    }
