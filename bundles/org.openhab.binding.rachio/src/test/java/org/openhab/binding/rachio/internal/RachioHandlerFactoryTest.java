...
class RachioHandlerFactoryTest {
    ...
    @Test
    void optionalCloudWebhookProviderCanBeAbsent() {
        RachioHandlerFactory factory = new RachioHandlerFactory();

        assertThat(factory.supportsThingType(THING_TYPE_CLOUD), is(true));
    }

    @Test
    void cloudWebhookProviderChangeNotifiesBridgeHandlers() throws Exception {
        RachioHandlerFactory factory = new RachioHandlerFactory();
        ...
        factory.updateCloudWebhookProvider(new Object());
        factory.updateCloudWebhookProvider(null);

        verify(bridgeHandler, Mockito.times(2)).onCloudWebhookProviderChanged();
    }

    @Test
    void nullCloudWebhookProviderServiceIsIgnored() throws Exception {
        RachioHandlerFactory factory = new RachioHandlerFactory();
        RachioBridgeHandler bridgeHandler = Mockito.mock(RachioBridgeHandler.class);
        RachioHandlerFactory.RachioBridge bridge = factory.new RachioBridge();
        bridge.cloudHandler = bridgeHandler;
        ...
        ServiceReference<Object> reference = serviceReference();

        assertThat(factory.addCloudWebhookProvider(bundleContext, reference), nullValue());

        verify(bridgeHandler, never()).onCloudWebhookProviderChanged();
        verify(bundleContext, never()).ungetService(reference);
    }

    @Test
    void trackedCloudWebhookProviderLifecycleNotifiesBridgeHandlers() throws Exception {
        RachioHandlerFactory factory = new RachioHandlerFactory();
        RachioBridgeHandler bridgeHandler = Mockito.mock(RachioBridgeHandler.class);
        RachioHandlerFactory.RachioBridge bridge = factory.new RachioBridge();
        bridge.cloudHandler = bridgeHandler;
        ...
        BundleContext bundleContext = Mockito.mock(BundleContext.class);
        ServiceReference<Object> reference = serviceReference();
        ServiceReference<Object> untrackedReference = serviceReference();
        Object webhookService = new Object();
        when(bundleContext.getService(reference)).thenReturn(webhookService);

        assertThat(factory.addCloudWebhookProvider(bundleContext, reference), is(webhookService));
        factory.removeCloudWebhookProvider(bundleContext, untrackedReference);
        factory.removeCloudWebhookProvider(bundleContext, reference);

        verify(bridgeHandler, Mockito.times(2)).onCloudWebhookProviderChanged();
        verify(bundleContext, never()).ungetService(untrackedReference);
