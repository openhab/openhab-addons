 ...
 */
package org.openhab.binding.midea.internal.discovery;
...
import org.openhab.binding.midea.internal.Utils;
import org.openhab.binding.midea.internal.cloud.CloudProvider;
import org.openhab.binding.midea.internal.devices.CommandBase;
import org.openhab.binding.midea.internal.devices.capabilities.CapabilityParser;
import org.openhab.binding.midea.internal.security.Security;
...
/**
 * The {@link MideaDiscoveryService} service for Midea AC.
 *
 * @author Jacek Dobrowolski - Initial contribution
 * @author Bob Eckhoff - OH naming conventions and Capabilities capture
 */
@NonNullByDefault
@Component(service = DiscoveryService.class, configurationPid = "discovery.midea")
public class MideaDiscoveryService extends AbstractDiscoveryService {
     ...
     */
    public static final int MIDEA_SEND_PORT1 = 6445;
    /**
     * UDP port2 to send command.
     */
    public static final int MIDEA_SEND_PORT2 = 20086;
    /**
     * UDP port devices send discover replies back.
     */
    public static final int MIDEA_RECEIVE_PORT = 6440;

    private final Logger logger = LoggerFactory.getLogger(MideaDiscoveryService.class);
    ...
    public void startDiscoverSocket(String ipAddress, @Nullable DiscoveryHandler discoveryHandler)
        ...
        discoverSocket = new DatagramSocket(new InetSocketAddress(MIDEA_RECEIVE_PORT));
        DatagramSocket discoverSocket = this.discoverSocket;
        if (discoverSocket != null) {
            ...
            final InetAddress broadcast = InetAddress.getByName(ipAddress);
            {
                final DatagramPacket discoverPacket = new DatagramPacket(CommandBase.discover(),
                        CommandBase.discover().length, broadcast, MIDEA_SEND_PORT1);
                discoverSocket.send(discoverPacket);
                logger.trace("Broadcast discovery package sent to port: {}", MIDEA_SEND_PORT1);
            }
            {
                final DatagramPacket discoverPacket = new DatagramPacket(CommandBase.discover(),
                        CommandBase.discover().length, broadcast, MIDEA_SEND_PORT2);
                discoverSocket.send(discoverPacket);
                logger.trace("Broadcast discovery package sent to port: {}", MIDEA_SEND_PORT2);
            }
        }
    }
