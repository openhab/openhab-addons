...
making manual changes. Do not rely on a global install; always use npx to ensure
...
- **CRITICAL: The SOLID principle applies to all files in this project, including source code, documentation, and diagrams.**
  - Every file (Java, XML, Markdown, etc.) must have a single, well-defined responsibility. If a file covers more than one concern, split it into separate files.
  - This includes, but is not limited to, documentation detail pages, source code classes, configuration files, and diagrams.
...
- **CRITICAL**: Agent MUST not automatically commit changes. All changes MUST be
  ...
  repository.
- **CRITICAL**: Every commit message MUST be reviewed by the developer for
  clarity, correctness, and compliance with project standards before the commit
  is finalized.
- **CRITICAL**: Every commit message MUST contain the "sign off" of the developer.
  The sign off line must be in the form
  `Signed-off-by: <Name> <email>`
  and must match the developer's git configuration.
  To retrieve the required name and email, the agent may use the following
