...
public class UserDataChangeInfo {
  ...
  public UserDataChangeInfo userId(@org.eclipse.jdt.annotation.Nullable
 UUID userId) {
    this.userId = userId;
    return this;
  }
 ...
 UUID userId) {
  ...
  public UserDataChangeInfo userDataList(@org.eclipse.jdt.annotation.Nullable
    ...
    if (getUserDataList() != null) {
      for (int i = 0; i < getUserDataList().size(); i++) {
        if (getUserDataList().get(i) != null) {
          joiner.add(getUserDataList().get(i).toUrlQueryString(String.format(java.util.Locale.ROOT, "%sUserDataList%s%s", prefix, suffix,
