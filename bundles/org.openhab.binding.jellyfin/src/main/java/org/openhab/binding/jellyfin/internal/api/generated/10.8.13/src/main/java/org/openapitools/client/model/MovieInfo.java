...
public class MovieInfo {
   ...
   * @throws IOException if the JSON Element is invalid with respect to MovieInfo
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!MovieInfo.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in MovieInfo is not found in the empty JSON string", MovieInfo.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!MovieInfo.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `MovieInfo` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!MovieInfo.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'MovieInfo' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<MovieInfo> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(MovieInfo.class));

       return (TypeAdapter<T>) new TypeAdapter<MovieInfo>() {
           @Override
           public void write(JsonWriter out, MovieInfo value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public MovieInfo read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of MovieInfo given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of MovieInfo
   * @throws IOException if the JSON string is invalid with respect to MovieInfo
   */
  public static MovieInfo fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, MovieInfo.class);
  }

  /**
   * Convert an instance of MovieInfo to an JSON string
   *
   * @return JSON string
   */
