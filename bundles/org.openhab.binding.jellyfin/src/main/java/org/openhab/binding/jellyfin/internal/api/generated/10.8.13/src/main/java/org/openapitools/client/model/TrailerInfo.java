...
public class TrailerInfo {
   ...
   * @throws IOException if the JSON Element is invalid with respect to TrailerInfo
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TrailerInfo.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in TrailerInfo is not found in the empty JSON string", TrailerInfo.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!TrailerInfo.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `TrailerInfo` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!TrailerInfo.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'TrailerInfo' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<TrailerInfo> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(TrailerInfo.class));

       return (TypeAdapter<T>) new TypeAdapter<TrailerInfo>() {
           @Override
           public void write(JsonWriter out, TrailerInfo value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public TrailerInfo read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of TrailerInfo given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of TrailerInfo
   * @throws IOException if the JSON string is invalid with respect to TrailerInfo
   */
  public static TrailerInfo fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, TrailerInfo.class);
  }

  /**
   * Convert an instance of TrailerInfo to an JSON string
   *
   * @return JSON string
   */
