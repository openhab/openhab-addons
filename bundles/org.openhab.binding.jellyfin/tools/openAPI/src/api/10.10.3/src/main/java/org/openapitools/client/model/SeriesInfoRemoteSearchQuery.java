...
public class SeriesInfoRemoteSearchQuery {
  ...
  public SeriesInfoRemoteSearchQuery includeDisabledProviders(@javax.annotation.Nullable Boolean includeDisabledProviders) {
   ...
   * @throws IOException if the JSON Element is invalid with respect to SeriesInfoRemoteSearchQuery
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SeriesInfoRemoteSearchQuery.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SeriesInfoRemoteSearchQuery is not found in the empty JSON string", SeriesInfoRemoteSearchQuery.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SeriesInfoRemoteSearchQuery.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SeriesInfoRemoteSearchQuery` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `SearchInfo`
      if (jsonObj.get("SearchInfo") != null && !jsonObj.get("SearchInfo").isJsonNull()) {
        SeriesInfo.validateJsonElement(jsonObj.get("SearchInfo"));
      }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SeriesInfoRemoteSearchQuery.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SeriesInfoRemoteSearchQuery' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SeriesInfoRemoteSearchQuery> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SeriesInfoRemoteSearchQuery.class));

       return (TypeAdapter<T>) new TypeAdapter<SeriesInfoRemoteSearchQuery>() {
           @Override
           public void write(JsonWriter out, SeriesInfoRemoteSearchQuery value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SeriesInfoRemoteSearchQuery read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of SeriesInfoRemoteSearchQuery given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of SeriesInfoRemoteSearchQuery
   * @throws IOException if the JSON string is invalid with respect to SeriesInfoRemoteSearchQuery
   */
  public static SeriesInfoRemoteSearchQuery fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SeriesInfoRemoteSearchQuery.class);
  }

  /**
   * Convert an instance of SeriesInfoRemoteSearchQuery to an JSON string
   *
   * @return JSON string
   */
