...
@NonNullByDefault
public class EvccPlanHandlerTest {
    ...
    @Nullable
    private EvccPlanHandler handler;

    private EvccPlanHandler createHandler() {
        return new EvccPlanHandler(thing, channelTypeRegistry, ZoneId.of("UTC")) {
            @Override
    ...
    @Test
    public void disablingOneTimePlanUsesDeleteEndpoint() {
        Objects.requireNonNull(handler).handleCommand(new ChannelUID("test:thing:uid:plan-active"),
                org.openhab.core.library.types.OnOffType.OFF);

        assertEquals(List.of("DELETE http://evcc/api/vehicles/vehicle_1/plan/soc"), requestedUrls);
    }
}
