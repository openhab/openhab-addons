...
public static final int CLUSTER_ID = 0x0453;
    ...
    /**
    * Indicates the Thread Extended PAN ID value for the Thread network designated by the user to be their preferred network for commissioning of Thread devices. If not null, the value of this attribute shall match the ExtendedPanID of a network in the ThreadNetworks attribute. A write operation with a non-null value that does not match any network in the ThreadNetworks list shall be rejected with a status of CONSTRAINT_ERROR.
...
On a factory reset this attribute shall be reset to null.
    */
    ...
    /**
    * Indicates the list of Thread Networks known about by this cluster. If the node hosting this cluster includes a Thread Border Router, then an entry for its Thread Network shall be included in this list.
...
On a factory reset this list shall be cleared, and any Thread Operational datasets previously stored shall be removed from the Node.
    */
    ...
    /**
    * Indicates the maximum number of entries that can be held in the ThreadNetworks list; it shall be at least 2 times the number of SupportedFabrics advertised in the Operational Credentials Cluster on the root endpoint of this node.
    */
    ...
    */
     public class ThreadNetworkStruct {
        ...
        */
        ...
        public ThreadNetworkStruct(String extendedPanId, String networkName, Integer channel, BigInteger activeTimestamp) {
