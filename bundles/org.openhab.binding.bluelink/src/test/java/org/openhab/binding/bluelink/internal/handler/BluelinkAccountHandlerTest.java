...
class BluelinkAccountHandlerTest extends JavaTest {
    ...
    void setUp() {
        ...
        handler = new BluelinkAccountHandler(bridge, HTTP_CLIENT, timeZoneProvider);
    ...
    @Test
    void testLoginAndGetVehicles() {
        waitForAssert(() -> assertThat(handler.getVehicles(), is(not(anEmptyMap()))));
        final VehicleInfo vehicle = handler.getVehicles().get("VIN1234");
    ...
    void testGetVehicleByVin() {
        waitForAssert(() -> assertThat(handler.getVehicles(), is(not(anEmptyMap()))));
        final VehicleInfo vehicle = handler.getVehicleByVin("VIN1234");
        assertNotNull(vehicle);
        assertEquals("VIN1234", vehicle.vin());
        assertEquals("IONIQ 6", vehicle.modelCode());

        assertNull(handler.getVehicleByVin("NONEXISTENT"));
    }
}
