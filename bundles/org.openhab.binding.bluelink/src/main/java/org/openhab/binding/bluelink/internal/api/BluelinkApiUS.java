...
import org.openhab.binding.bluelink.internal.dto.us.EnrollmentResponse;
import org.openhab.binding.bluelink.internal.dto.us.EnrollmentResponse.EnrolledVehicle;
import org.openhab.binding.bluelink.internal.dto.us.LoginRequest;
import org.openhab.binding.bluelink.internal.dto.us.VehicleInfoUS;
 ...
 */
@NonNullByDefault
public class BluelinkApiUS implements BluelinkApi {
    ...
    private final Logger logger = LoggerFactory.getLogger(BluelinkApiUS.class);
    ...
    public BluelinkApiUS(final HttpClient httpClient, final TimeZoneProvider timeZoneProvider, final String username,
            final String password, final @Nullable String pin) {
     ...
     */
    public BluelinkApiUS(final HttpClient httpClient, final String baseUrl, final TimeZoneProvider timeZoneProvider,
    ...
    public List<Vehicle> getVehicles() throws BluelinkApiException {
        ...
        try {
            ...
            return vehicles.stream().map(EnrolledVehicle::vehicleDetails).filter(Objects::nonNull)
                    .map(VehicleInfoUS::mapToVehicle).toList();
        } catch (final InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    public @Nullable VehicleStatus getVehicleStatus(final Vehicle vehicle, final boolean forceRefresh)
        ...
        try {
            ...
            return VehicleStatusMapper.map(gson.fromJson(response.getContentAsString(), VehicleStatusUS.class),
                    new QuantityType<>(vehicle.odometer(), ImperialUnits.MILE));
        } catch (final InterruptedException e) {
            Thread.currentThread().interrupt();
    ...
    private boolean sendDoorCommand(final Vehicle vehicle, final String endpoint) throws BluelinkApiException {
        ensureAuthenticated();
    ...
    public boolean climateStop(final Vehicle vehicle) throws BluelinkApiException {
        ensureAuthenticated();
        return sendSimplePostCommand(vehicle, vehicle.electric() ? "/ac/v2/evc/fatc/stop" : "/ac/v2/rcs/rsc/stop");
    }

    private boolean sendClimateCommand(final Vehicle vehicle, final String url, final Object climateRequest)
            throws BluelinkApiException {
        try {
            final Request request = httpClient.newRequest(url).method(HttpMethod.POST).timeout(HTTP_TIMEOUT_SECONDS,
                    TimeUnit.SECONDS);
    ...
    @Override
    public boolean setChargeLimitDC(final Vehicle vehicle, final int limit) throws BluelinkApiException {
        return setChargeLimit(vehicle, PLUG_TYPE_DC, limit);
    }

    @Override
    public boolean setChargeLimitAC(final Vehicle vehicle, final int limit) throws BluelinkApiException {
        return setChargeLimit(vehicle, PLUG_TYPE_AC, limit);
    }

    private boolean setChargeLimit(final Vehicle vehicle, final int plugType, final int limit)
            throws BluelinkApiException {
        ensureAuthenticated();
    ...
    private boolean sendSimplePostCommand(final Vehicle vehicle, final String endpoint) throws BluelinkApiException {
        final String url = baseUrl + endpoint;
    ...
    private void addVehicleHeaders(final Request request, final Vehicle vehicle) {
        final String regId = vehicle.registrationId();
        final String gen = vehicle.generation() != null ? String.valueOf(vehicle.generation()) : null;
        final String vin = vehicle.vin();
