...
* Force Refresh: Requests up-to-date data from the vehicle and updates the channels.
* Climate Start
* Climate Stop
...
| `refreshInterval` | No       | 30      | Status refresh interval in minutes  |

If `refreshInterval` is too aggressive, you may hit the daily rate limit.
...
|------------------|----------------------|------------|--------------------------------------------|
| `locked`         | Switch               | RW         | Door lock status (ON=locked, OFF=unlocked) |
...
| Channel              | Type               | Read/Write | Description                  |
|----------------------|--------------------|------------|------------------------------|
| `hvac-on`            | Switch             | RW         | Climate control on/off       |
| `temperature`        | Number:Temperature | R          | Temperature setting          |
...
| `low-warning` | Switch               | R          | Low fuel warning    |
...
| `charging`              | Switch               | RW         | Charging status (ON to start)   |
| `plugged-in`            | Switch               | R          | Charge cable plugged in         |
| `charge-limit-dc`       | Number:Dimensionless | RW         | DC charge limit (%)             |
| `charge-limit-ac`       | Number:Dimensionless | RW         | AC charge limit (%)             |
...
// Climate
Number:Temperature Car_Temp  "Temperature [%.0f %unit%]"    <temperature> { channel="bluelink:vehicle:myaccount:ioniq5:climate#temperature" }
...
Switch  Car_Fuel_Low         "Fuel Low [%s]"                             { channel="bluelink:vehicle:myaccount:ioniq5:fuel#low-warning" }
