...
public class EnergyInfo {
    ...
    public static EnergyInfo parse(String response) {
        ...
        // responseMap.forEach((k,v) -> logger.debug("Key = {}, value {}", k, v));
        if(responseMap.get("ret") != null && responseMap.get("ret").contentEquals("OK")) {
            ...
            logger.debug("energyCoolingToday {}", info.energyCoolingToday);
            logger.debug("energyCoolingThisWeek {}", info.energyCoolingThisWeek );
            logger.debug("energyCoolingLastWeek {}", info.energyCoolingLastWeek );              
