...
public class UnifiAccessDoorHandler extends UnifiAccessBaseHandler {
    ...
    /**
     * Returns the hub device ID for lock rule commands. The v2 API uses the hub's
     * unique_id for lock_rule endpoints, not the door/location ID. Falls back to
     * the door ID if no hub is known.
     */
    private String getHubDeviceId() {
        String hubId = door.hubDeviceId;
        return hubId != null ? hubId : deviceId;
    }
    ...
    protected void handleLocationState(LocationState locationState) {
        ...
        // Fire alarm triggers (e.g., force_unlock when door is forced open while locked)
        if (locationState.alarms != null) {
            for (var alarm : locationState.alarms) {
                if (alarm.type != null) {
                    triggerChannel(UnifiAccessBindingConstants.CHANNEL_DOOR_ALARM, alarm.type);
                }
            }
        }
    }
    ...
    protected void updateFromDoor(Door door) {
        ...
        if (door.fullName != null) {
            properties.put("fullName", door.fullName);
        }
        if (door.floorId != null) {
            properties.put("floorId", door.floorId);
        }
        if (door.isBindHub != null) {
            properties.put("isBindHub", String.valueOf(door.isBindHub));
        }
        ...
        if (thumbPath != null) {
            ...
            if (api != null) {
                try {
                    ...
                    logger.debug("Failed to get initial door thumbnail: {}", e.getMessage());
                }
            }
        }
        UniFiAccessApiClient api = getApiClient();
