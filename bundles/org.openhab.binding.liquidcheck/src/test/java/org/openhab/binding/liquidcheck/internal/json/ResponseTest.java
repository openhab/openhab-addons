...
public class ResponseTest {
    ...
    @DisplayName("Polling for data response test")
    @Test
    public void responseTest() {
        ...
        Response response = this.response != null ? this.response : new Response();
        if (response != null) {
            ...
            assertThat(response.context.status, is(equalTo("")));
        }
    }

    @DisplayName("Command response test")
    @Test
    public void commandResponseTest() {
        try {
            List<String> lines = Files.readAllLines(Paths.get("src/test/resources/CommandResponse.json"),
                    StandardCharsets.UTF_8);
            StringBuilder sb = new StringBuilder();
            for (String line : lines) {
                sb.append(line);
        ...
        if (response != null) {
            ...
            assertThat(response.header.messageId, is(equalTo("016D3C6E-E19D0D1D")));
            assertThat(response.header.payloadVersion, is(equalTo("1")));
            assertThat(response.header.authorization, is(equalTo("1C9DC262BE70-000EB80B-0JB3OZ92QFQHJNDROPWFFLZ")));
            assertThat(response.payload.measure.level, is(equalTo(0.0)));
            assertThat(response.payload.measure.content, is(equalTo(0)));
            assertThat(response.payload.measure.age, is(equalTo(0)));
            assertThat(response.payload.measure.raw.level, is(equalTo(0.0000)));
            assertThat(response.payload.measure.raw.content, is(equalTo(0.0000)));
