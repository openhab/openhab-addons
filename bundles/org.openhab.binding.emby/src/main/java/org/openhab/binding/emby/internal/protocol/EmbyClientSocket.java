...
public class EmbyClientSocket {
    ...
    public class EmbyWebSocketListener {
        ...
        public void onClose(int statusCode, @Nullable String reason) {
            String message = "";
            if (reason == null) {
                message = "The message was null in the closing of the websocket.";
            } else {
                message = reason;
            }

            logger.debug("Closing a WebSocket due to {} with status code: {}", message, Integer.toString(statusCode));
            session = null;
            connected = false;
            if (eventHandler != null) {
                scheduler.submit(() -> {
