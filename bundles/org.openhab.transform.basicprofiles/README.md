...
For the second part we use the `JS` transformation with a `doorlock.js`:

```
(function(i) {
    return JSON.stringify({ device: { command: i } })
})(input)
```
...
```
MAP:doorlock.map∩JS:doorlock.js
```
