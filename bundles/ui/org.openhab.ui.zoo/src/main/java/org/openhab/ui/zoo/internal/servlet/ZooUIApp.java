...
public class ZooUIApp {
	...
	protected void activate(ComponentContext componentContext) {
		try {
			...
			final String uri = getInfluxUri();
			String[] credentials = getCredentialsFromUri(uri);

			initParamsProxy.put("targetUri", uri);
			initParamsProxy.put("log", "true");
			httpService.registerServlet("/zoo/influxproxy", new InfluxProxyServlet(credentials[0], credentials[1]), initParamsProxy, null);
	...
	private String[] getCredentialsFromUri(final String uri) throws URISyntaxException {
		final URI influxUri = new URI(uri);
		final String userInfo = influxUri.getUserInfo();
		if (userInfo == null) {
			logger.info("No credentials for InfluxDB on URL found. Example: {}", DEFAULT_INFLUX_URI);
		}
		if (userInfo != null) {
	...
	private boolean isDebugModeEnabled() {
		...
		if (!distFolderExists) {
			...
			logger.debug("Zoo UI found a /{} folder, but forced to use /{} because of system parameter '{}'.", WEB_DIST_FOLDER, WEB_FOLDER,
