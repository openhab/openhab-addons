...
1. **Transit Route Details (`routedetails`)**
   ...
   - Channels:
     ...
     - `route#alertTitle` / `route#alertDescription` / `route#alertSeverity` (String): Active service alerts and disruptions
...
1. **Transit Trip Details (`tripdetails`)**
   ...
   - Upcoming Stop Channels (`stop1` to `stop5`):
     ...
     - `stopX#scheduledTime` (DateTime): Scheduled timetable arrival/departure time
     - `stopX#realtimeTime` (DateTime): Predicted real-time arrival/departure time
     - `stopX#delayMinutes` (String): Current delay in minutes (e.g., "+3", "0", "-1")
...
DateTime Stop1_Time      "Abfahrt [%1$tH:%1$tM]" { channel="transitapp:stop:mybridge:mystop:depart1#departureTime" }
...
String Trip_NextDelay    "Verspätung [%s min]" { channel="transitapp:tripdetails:mybridge:mytrip:stop1#delayMinutes" }
```
