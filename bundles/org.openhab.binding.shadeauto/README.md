...
```java
Bridge shadeauto:hub:myhub "ShadeAuto Hub" [ host="192.168.1.100", pollingInterval=1800 ] {
...
```java
Dimmer               BedroomShade_Position     "Bedroom Shade [%d %%]"       { channel="shadeauto:shade:myhub:bedroom:position" }
Number:Dimensionless BedroomShade_Battery      "Bedroom Battery [%d %%]"     { channel="shadeauto:shade:myhub:bedroom:batteryLevel" }
Switch               BedroomShade_LowBattery   "Bedroom Low Battery"         { channel="shadeauto:shade:myhub:bedroom:lowBattery" }
Switch               Hub_ForceRefresh          "Force Refresh"               { channel="shadeauto:hub:myhub:forceRefresh" }
```

### shadeauto.sitemap

```perl
sitemap shadeauto label="ShadeAuto" {
    Frame label="Bedroom" {
