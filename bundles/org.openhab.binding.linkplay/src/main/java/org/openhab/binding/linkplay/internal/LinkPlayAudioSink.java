...
public class LinkPlayAudioSink extends AudioSinkSync {
    ...
    public CompletableFuture<@Nullable Void> processAndComplete(@Nullable AudioStream audioStream) {
        ...
        if (audioStream instanceof URLAudioStream urlAudioStream) {
            // Asynchronous handling for URLAudioStream - play and close stream
            CompletableFuture<@Nullable Void> future = playMedia(urlAudioStream.getURL());
            try {
                urlAudioStream.close();
            } catch (IOException e) {
                logger.trace("Error closing audio stream: {}", e.getMessage());
            }
            return future;
        } else {
        ...
        if (callbackUrl != null) {
            ...
            try {
                ...
                try {
                    ...
                    logger.trace("Error closing audio stream: {}", ex.getMessage());
                }
            ...
            logger.debug("We do not have any callback url, so cannot play the audio stream!");
            try {
                audioStream.close();
            } catch (IOException e) {
                logger.trace("Error closing audio stream: {}", e.getMessage());
