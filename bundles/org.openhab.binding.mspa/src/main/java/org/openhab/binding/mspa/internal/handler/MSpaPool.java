...
public class MSpaPool extends BaseThingHandler {
     ...
     * affected here - only the {@code fault} channel reflects the condition. When the code is blank, the special
     * value {@link org.openhab.binding.mspa.internal.MSpaConstants#FAULT_STATE_NONE} is reported, which the
    ...
    private void updateFaultState(JSONObject rawData) {
        if (rawData.has("fault")) {
            String fault = rawData.getString("fault");
