...
public class HeatingCircuitHandler extends BaseThingHandler {
    ...
    private final HeatingCircuitBlockParser heating_circuitBlockParser = new HeatingCircuitBlockParser();
    private final HeatingCircuitReg50BlockParser heating_circuitReg50BlockParser = new HeatingCircuitReg50BlockParser();

    /**
     * These are the tasks used to poll the device
     */

    private volatile @Nullable AbstractBasePoller heating_circuitPoller = null;
    private volatile @Nullable AbstractBasePoller heating_circuitReg50Poller = null;
    /**
            ...
            if (groupId != null) {
                ...
                switch (groupId) {
                    case GROUP_HEATING_CIRCUIT:
                        poller = heating_circuitPoller;
                        break;
                    case GROUP_HEATING_CIRCUIT_REG50:
                        poller = heating_circuitReg50Poller;
                        break;
                    default:
                        poller = null;
                        break;
    ...
    private void startUp() {
        ...
        if (config == null) {
            logger.debug("Invalid comms/config/manager ref for lambda heating_circuitcirciut handler");
            return;
        }
        ...
        if (heating_circuitPoller == null) {
            ...
            heating_circuitPoller = poller;
        }
        if (heating_circuitReg50Poller == null) {
            AbstractBasePoller poller = new AbstractBasePoller() {
                @Override
                protected void handlePolledData(ModbusRegisterArray registers) {
            ...
            heating_circuitReg50Poller = poller;
        }
    ...
    private void tearDown() {
        ...
        AbstractBasePoller poller = heating_circuitPoller;
        if (poller != null) {
            poller.unregisterPollTask();
            heating_circuitPoller = null;
        }

        poller = heating_circuitReg50Poller;
        if (poller != null) {
            poller.unregisterPollTask();
            heating_circuitReg50Poller = null;
        }
        comms = null;
    }
    ...
    protected void handlePolledHeatingReg50Data(ModbusRegisterArray registers) {
        ...
        HeatingCircuitReg50Block block = heating_circuitReg50BlockParser.parse(registers);
        ...
        updateState(channelUID(GROUP_HEATING_CIRCUIT_REG50, CHANNEL_HEATING_CIRCUIT_OFFSET_FLOW_LINE_TEMPERATURE),
                getScaled(block.heating_circuitOffsetFlowLineTemperature, CELSIUS, -1.0));
        updateState(channelUID(GROUP_HEATING_CIRCUIT_REG50, CHANNEL_HEATING_CIRCUIT_ROOM_HEATING_TEMPERATURE),
                getScaled(block.heating_circuitRoomHeatingTemperature, CELSIUS, -1.0));
        updateState(channelUID(GROUP_HEATING_CIRCUIT_REG50, CHANNEL_HEATING_CIRCUIT_ROOM_COOLING_TEMPERATURE),
                getScaled(block.heating_circuitRoomCoolingTemperature, CELSIUS, -1.0));
        resetCommunicationError();
    }
