...
public class HeatpumpHandler extends BaseThingHandler {
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        if (RefreshType.REFRESH == command) {
            ...
            try {
                ...
                if (GROUP_HEATPUMP.equals(channelUID.getGroupId())) {

                    switch (channelUID.getIdWithoutGroup()) {

                        case CHANNEL_HEATPUMP_REQUEST_T_FLOW:
                            writeInt16(baseadress + 16, getScaledInt16Value(command));
                            break;

                        case CHANNEL_HEATPUMP_REQUEST_T_RETURN:
                            writeInt16(baseadress + 17, getScaledInt16Value(command));
                            break;

                        case CHANNEL_HEATPUMP_REQUEST_HEAT_SINK:
                            writeInt16(baseadress + 18, getScaledInt16Value(command));
                            break;
                    }
                } else if (GROUP_HEATPUMP_REG50.equals(channelUID.getGroupId())) {
    ...
    protected void handlePolledHeatpumpData(ModbusRegisterArray registers) {
        ...
        updateState(channelUID(GROUP_HEATPUMP, CHANNEL_HEATPUMP_REQUEST_T_FLOW),
                getScaled(block.heatpumpRequestTFlow, CELSIUS, -1.0));
        updateState(channelUID(GROUP_HEATPUMP, CHANNEL_HEATPUMP_REQUEST_T_RETURN),
                getScaled(block.heatpumpRequestTReturn, CELSIUS, -1.0));
        updateState(channelUID(GROUP_HEATPUMP, CHANNEL_HEATPUMP_REQUEST_HEAT_SINK),
