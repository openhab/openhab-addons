...
public class VWWeConnectSession {
    ...
    private synchronized boolean updateVehicleStatus(Class<? extends Vehicle> jsonClass) {
        ...
        if (context == null) {
            logger.warn("No Fully loaded vehicle(s) found on VW We Connect portal!!");
            return false;
        }
        ...
        if (vinList == null || vinList.size() == 0 || dashboardUrlList == null) {
            logger.warn("No Fully loaded vehicle(s) found on VW We Connect portal! Document context: {}", context);
            return false;
        }

        if (vinList.size() != dashboardUrlList.size()) {
            logger.warn("VIN list {} is not equal to dashboard URL list {} on VW We Connect portal!", vinList,
                    dashboardUrlList);
            return false;
        }
