...
Configure via `services/timescaledb.cfg` or in mainUI under `Settings → Other Services → TimescaleDB Persistence`.
...
|----------------------|-----------------------------|----------------------------------------------------------------|
| value (main)         | `AVG`, `MAX`, `MIN`, `SUM`  | Aggregation function applied during downsampling               |
| `downsampleInterval` | e.g. `1h`, `15m`, `1d`      | Time bucket size for aggregation                               |
...
// No downsampling, but keep only 30 days
