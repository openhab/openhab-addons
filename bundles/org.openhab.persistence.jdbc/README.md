...
| tableDoubleQuotedTableNames | `true` for `postgresql` and `timescaledb`, otherwise `false` |    No     | When set to `true`, then the name of SQL table is double quoted. Note: for backward compatibility the default is `true` when the `<service>` part of the `url` configuration parameter is either `postgresql` or `timescaledb`, otherwise the default is `false`. |
