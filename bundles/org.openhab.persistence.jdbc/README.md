...
| tablePreserveCase         | `false`                                                      |    No     | table name case when `tableUseRealItemNames` is `true`. When set to `true`, item name case is preserved in table table names. When set to `false`, tables names are lower cased. |
