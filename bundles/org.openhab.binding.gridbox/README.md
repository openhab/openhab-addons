...
To switch between Viessmann and E.ON authentication, set the "eOnClientType" parameter to true (for E.ON) or false (for Viessmann).
The Viessmann service will be migrated to E.ON and is planned to be stopped after the 31.12.2025. After that date, only the E.ON 
...
| eOnClientType   | boolean | Use the E.ON (true) or Viessmann (false) API      | true    | no       | no       |
