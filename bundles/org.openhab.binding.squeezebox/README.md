...
Other versions followed, gradually adding native support for additional file formats, Wi‑Fi support, larger and more advanced displays, as well as a version targeting audiophile users.
Support for playing music from external streaming platforms such as Pandora, Napster, Last.fm and Sirius were also added.
The devices generally have two operating modes: either standalone, where the device connects directly to an internet streaming service, or connected to a local computer running the Logitech Media Server or a network‑attached storage device.
...
Given the cross‑platform nature of the server and software client, some users have ensured the continued use of the platform by utilizing the Raspberry Pi as a dedicated Squeezebox device (both client and server).
...
Squeeze players may be official Logitech products or other players like [Squeezelite](https://code.google.com/p/squeezelite/).

## Discovery

A Squeeze Server is discovered through UPnP on the local network.
...
By default, the binding sends the Squeezebox the URL for retrieving the audio stream based on the Primary Address (Network Settings configuration) and the openHAB HTTP port.
...
The Squeeze Server bridge requires the IP address, web port, and CLI port.
If Squeeze Server authentication is enabled, the user ID and password are also required.
...
| control                | Player      | Control the Zone Player, e.g. play/pause/next/previous/fast‑forward/rewind                    |
| stream                 | String      | Play the given HTTP or file stream (file:// or http://)                                       |
| source                 | String      | Shows the source of the currently playing playlist entry (e.g., `http://radio.org/radio.mp3`) |
...
- In the Selection widget settings
  ...
  - Select _Choices source_ of _Server-provided item options_
    - Modify other settings to suit your taste
...
Some streaming services, such as Pandora and Slacker, allow songs to be rated.
