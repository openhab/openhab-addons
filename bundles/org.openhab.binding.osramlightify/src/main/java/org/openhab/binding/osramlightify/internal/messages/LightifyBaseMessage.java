...
public abstract class LightifyBaseMessage {
    ...
    private @Nullable IEEEAddress deviceAddress = null;

    public LightifyBaseMessage(@Nullable LightifyDeviceHandler deviceHandler, Command command) {
    ...
    protected ByteBuffer encodeMessage(int length) throws LightifyMessageTooLongException {
        int messageLength = HEADER_LENGTH + (deviceAddress != null ? IEEEAddress.ADDRESS_LENGTH : 0) + length;
