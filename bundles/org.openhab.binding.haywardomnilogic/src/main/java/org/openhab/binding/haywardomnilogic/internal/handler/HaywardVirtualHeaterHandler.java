 ...
 */
@NonNullByDefault
public class HaywardVirtualHeaterHandler extends BaseThingHandler {
    ...
    public void updateVirtualHeaterData(String systemID, String channelID, String data) {
        Channel chan = getThing().getChannel(channelID);
        if (chan != null) {
            if (data.equals("yes")) {
                updateState(chan.getUID(), new DecimalType(1));
            } else if (data.equals("no")) {
                updateState(chan.getUID(), new DecimalType(0));
            } else {
                updateState(chan.getUID(), new DecimalType(data));
            }
            logger.trace("Updated Hayward Virtual Heater {} {} to: {}", systemID, channelID, data);
        }
    }
