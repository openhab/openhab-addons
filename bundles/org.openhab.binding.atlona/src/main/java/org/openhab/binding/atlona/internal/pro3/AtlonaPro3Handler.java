            ...
            switch (id) {
                case AtlonaPro3Constants.CHANNEL_POWER:
                    if (command instanceof OnOffType type) {
                        final boolean makeOn = type == OnOffType.ON;
                ...
                try {
                    ...
                    switch (id) {
                        case AtlonaPro3Constants.CHANNEL_PORTOUTPUT:
                            if (command instanceof DecimalType port) {
                                final int inpNbr = port.intValue();
                ...
                try {
                    ...
                    switch (id) {
                        case AtlonaPro3Constants.CHANNEL_PORTMIRROR:
                            if (command instanceof DecimalType port) {
                                final int outPortNbr = port.intValue();
                                if (outPortNbr <= 0) {
                ...
                try {
                    ...
                    switch (id) {
                        case AtlonaPro3Constants.CHANNEL_VOLUME_MUTE:
                            if (command instanceof OnOffType type) {
                                atlonaHandler.setVolumeMute(portNbr, type == OnOffType.ON);
                            } else {
                        ...
                        case AtlonaPro3Constants.CHANNEL_VOLUME:
                            if (command instanceof DecimalType volume) {
                                final int level = volume.intValue();
