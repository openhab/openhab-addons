...
public class LinkDB {
    ...
    public int getFirstRecordComponentId() {
        return Objects
                .requireNonNull(Optional.ofNullable(getFirstRecord()).map(LinkDBRecord::getComponentId).orElse(0));
    }
    ...
    public int getNextAvailableLocation() {
        return Objects.requireNonNull(
                getRecords().stream().filter(LinkDBRecord::isAvailable).map(LinkDBRecord::getLocation).findFirst()
