...
import org.openhab.binding.insteon.internal.database.LinkDBChange;
...
import org.openhab.binding.insteon.internal.database.ModemDBChange;
...
public class InsteonDevice extends BaseDevice<InsteonAddress, InsteonDeviceHandler> {
    ...
    public double getLastMsgValueAsDouble(String type, int group, double defaultValue) {
        return Objects.requireNonNull(Optional.ofNullable(getFeature(type, group)).map(DeviceFeature::getLastMsgValue)
     ...
     */
    public void adjustResponders(InsteonAddress address, int group, InsteonV2ChannelConfiguration config, Command cmd) {
        // handle command for responder feature with group matching record component id (data 3)
        linkDB.getResponderRecords(address, group)
                .forEach(record -> getResponderFeatures().stream()
                        .filter(feature -> feature.getComponentId() == record.getComponentId()).findFirst()
                        .ifPresent(feature -> {
                            InsteonV2ChannelConfiguration adjustConfig = InsteonV2ChannelConfiguration.copyOf(config,
     ...
     * @param cmd the cmd to adjust to
     */
    public void adjustRelatedDevices(int group, InsteonV2ChannelConfiguration config, Command cmd) {
        InsteonModem modem = getModem();
        if (modem != null) {
            linkDB.getRelatedDevices(group).stream().map(modem::getInsteonDevice).filter(Objects::nonNull)
                    .map(Objects::requireNonNull).forEach(device -> {
    ...
    public int getBroadcastGroup(DeviceFeature feature) {
        ...
        if (modem != null) {
            ...
            return Objects.requireNonNull(linkDB.getBroadcastGroups(feature.getComponentId()).stream()
