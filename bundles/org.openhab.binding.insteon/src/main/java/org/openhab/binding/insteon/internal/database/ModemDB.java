...
public class ModemDB {
    ...
    private int getRecordIndex(InsteonAddress address, int group, boolean isController) {
        return Objects.requireNonNull(
                getRecords(address, group, isController).findFirst().map(this::getRecordIndex).orElse(-1));
    }

    private int getRecordIndex(InsteonAddress address, int group) {
        return Objects
                .requireNonNull(getRecords(address, group, null).findFirst().map(this::getRecordIndex).orElse(-1));
    }
    ...
    private int getChangeIndex(InsteonAddress address, int group, boolean isController) {
        return Objects.requireNonNull(
                getChanges(address, group, isController).findFirst().map(this::getChangeIndex).orElse(-1));
    }
