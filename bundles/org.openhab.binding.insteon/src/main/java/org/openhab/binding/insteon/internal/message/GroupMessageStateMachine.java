...
public class GroupMessageStateMachine {
     ...
     */
    public boolean action(GroupMessageType type, InsteonAddress address, int group, byte cmd1) {
        publish = false;
                        ...
                        if (lastCmd1 != cmd1) {
                            if (logger.isDebugEnabled()) {
                                logger.debug("{} group {} cmd1 {} is not a dup BCAST, last cmd1 {}", address, group,
                                        ByteUtils.getHexString(cmd1), ByteUtils.getHexString(lastCmd1));
                            }
                            publish = true;
                        } else if (currentTime > lastUpdated + GROUP_STATE_TIMEOUT) {
                            if (logger.isDebugEnabled()) {
                                logger.debug("{} group {} cmd1 {} is not a dup BCAST, last state {} timed out", address,
                                        group, ByteUtils.getHexString(cmd1), state);
                            }
                            publish = true;
                        } else {
                            publish = false;
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("{} group {} state: {} --{}--> {}, publish: {}", address, group, oldState, type, state,
                    publish);
        }
        return publish;
    }
