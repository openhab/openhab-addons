...
class VisualCrossingApiTest {
    ...
    @BeforeEach
    void setUp() {
        api = new VisualCrossingApi("https://weather.visualcrossing.com", "xyz",requireNonNull(restClient), new Gson());
    }
    ...
    void parseFullJson() throws VisualCrossingAuthException, VisualCrossingApiException, VisualCrossingRateException {
        ...
        given(requireNonNull(restClient).get("https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/"//
                + "wroc%C5%82aw%2Cpoland"//
                + "?key=xyz"//
                + "&contentType=json"//
                +"&lang=en"//
                + "&unitGroup=metric")).willReturn(FULL_JSON);
