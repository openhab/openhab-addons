 ...
 */
package org.openhab.binding.panasonictv.handler;

import static org.openhab.binding.panasonictv.PanasonicTvBindingConstants.POWER;
...
public class PanasonicTvHandler extends BaseThingHandler implements DiscoveryListener, RegistryListener, EventListener {
    ...
    private PanasonicTvConfiguration configuration;
    ...
    public void handleCommand(ChannelUID channelUID, Command command) {
        ...
        for (PanasonicTvService service : services) {
    ...
    public void thingDiscovered(DiscoveryService source, DiscoveryResult result) {
        ...
        if (configuration.hostName.equals(result.getProperties().get(HOST_NAME))) {
            /*
             * TV discovery services creates thing UID from UPnP UDN.
    ...
    /**
     * One TV contains several UPnP devices. TV is discovered by
     * Media Renderer UPnP device. This polling job tries to find another UPnP
     * devices related to same TV and create handler for those.
     */
        ...
        if (configuration != null) {
            if (configuration.hostName.equals(device.getIdentity().getDescriptorURL().getHost())) {
                ...
                if (existingService == null || !existingService.isUpnp()) {
                    PanasonicTvService newService = ServiceFactory.createService(type, upnpIOService, upnpService, udn,
    ...
    private PanasonicTvService findServiceInstance(String serviceName) {
        Class<? extends PanasonicTvService> cl = ServiceFactory.getClassByServiceName(serviceName);
