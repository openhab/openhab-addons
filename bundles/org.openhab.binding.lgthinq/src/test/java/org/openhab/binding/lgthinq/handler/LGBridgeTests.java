...
class LGBridgeTests {
    ...
    private String sessionTokenReturned =  "{\"status\":1,\"access_token\":\""+accessToken+"\",\"expires_in\":\"3600\",\"refresh_token\":\""+refreshToken+"\",\"oauth2_backend_url\":\"http://localhost:8880/\"}";
...
//    }
    ...
    public void testDiscoveryThings() {
        ...
        stubFor(post("/spx"+PRE_LOGIN_PATH)
                .withRequestBody(containing("user_auth2="+preLoginPwd))
                .willReturn(ok(preLoginResponse)));
        URI uri = UriBuilder.fromUri("http://localhost:8880").path("spx"+OAUTH_SEARCH_KEY_PATH)
                .queryParam("key_name", "OAUTH_SECRETKEY")
                .queryParam("sever_type","OP").build(); // YES! It's sever_type, not server_type 8-|
        stubFor(get(String.format("%s?%s", uri.getPath(), uri.getQuery()))
                .willReturn(ok(oauthTokenSearchKeyReturned)));
        stubFor(post(V2_SESSION_LOGIN_PATH + fakeUserName)
                .withRequestBody(containing("user_auth2=SOME_DUMMY_ENC_PWD"))
                .withHeader("X-Signature", equalTo("SOME_DUMMY_SIGNATURE"))
                .withHeader("X-Timestamp", equalTo("1643236928"))
                .willReturn(ok(loginSessionResponse)));
        ...
        stubFor(post("/emp/oauth2/token/empsession")
                .withRequestBody(containing("account_type="+userIdType))
...
//                .withHeader("lgemp-x-signature", equalTo(tokenSign)) // ignored because of timestamp in different times.
                .withHeader("lgemp-x-session-key", equalTo(loginSessionId))
//                .withHeader("lgemp-x-date", equalTo(currTimestamp)) // ignored because of timestamp in different times.
