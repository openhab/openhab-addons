...
/**
 * The {@link SceneEntity} class represents a scene entity type in the Home Assistant binding.
 * Maps Home Assistant scenes to openHAB Switch channels to allow scene activation.
 *
 * @author Jimmy Tanagra - Initial contribution
 */
@NonNullByDefault
public class SceneEntity implements EntityType {
    ...
    public Map<String, ParsedData> parseState(EntityState entityState, EntityContext context) {
        // Scenes are typically activated, not maintained, so we return OFF as the default state.
        return Map.of(EntityType.PRIMARY_ATTR, ParsedData.of(OnOffType.OFF));
    }
    ...
    public Optional<ServiceCall> toServiceCall(String entityId, String attribute, Command command,
        ...
        if (EntityType.isPrimary(attribute) && command == OnOffType.ON) {
            return CommandMapper.onOff(command, "scene", entityId);
        }
