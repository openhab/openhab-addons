...
/**
 * The {@link SelectEntity} class represents a select entity type in the Home Assistant binding.
 * Maps Home Assistant select dropdown options to openHAB String channels and handles option selection.
 *
 * @author Jimmy Tanagra - Initial contribution
 */
@NonNullByDefault
public class SelectEntity implements EntityType {
    ...
    public @Nullable StateDescriptionFragment getStateDescriptionFragment(EntityState entityState, String attribute,
        ...
        if (EntityType.isPrimary(attribute)) {
            return OptionUtils.extractStateOptions(entityState, "options");
        }
        return null;
    }
    ...
    public Optional<ServiceCall> toServiceCall(String entityId, String channelSuffix, Command command,
        ...
        if (EntityType.isPrimary(channelSuffix)) {
            return CommandMapper.onString(command, getType(), "select_option", "option", entityId);
        }
