...
public abstract class CulCunBaseBridgeHandler extends BaseBridgeHandler
    ...
    protected @Nullable CULConfig culConfig;
    private CULManager manager;
    private @Nullable CULHandlerInternal cul;
    ...
    protected synchronized void connect() {
        CULConfig culConfig = this.culConfig;
        if (culConfig == null) {
            updateStatus(ThingStatus.OFFLINE, ThingStatusDetail.CONFIGURATION_ERROR);
            return;
        }
        try {
            CULHandlerInternal cul = manager.getOpenCULHandler(culConfig);
    ...
    @Override
    public void dispose() {
        CULHandlerInternal cul = this.cul;
        if (cul != null) {
