...
class HomekitThermostatImpl extends AbstractHomekitAccessoryImpl<GroupItem> implements ThermostatAccessory {
    ...
    public CompletableFuture<CurrentHeatingCoolingStateEnum> getCurrentState() {
        ...
        final Optional<HomekitTaggedItem> characteristic = getMandatoryCharacteristic(HomekitCharacteristicType.CURRENT_HEATING_COOLING_STATE);
        if (characteristic.isPresent()) {
            stringValue = characteristic.get().getItem().getState().toString();
        } else {
            LOGGER.error("Missing mandatory characteristic {}",HomekitCharacteristicType.CURRENT_HEATING_COOLING_STATE);
        }
    ...
    public CompletableFuture<Double> getCurrentTemperature() {
        final Optional<HomekitTaggedItem> characteristic = getMandatoryCharacteristic(HomekitCharacteristicType.CURRENT_TEMPERATURE);
        if (characteristic.isPresent()) {
            DecimalType state = characteristic.get().getItem().getStateAs(DecimalType.class);
            if (state != null) {
                return CompletableFuture.completedFuture(convertToCelsius(state.doubleValue()));
            } else {
                LOGGER.error("The state of the item {} null",characteristic.get().getItem().getName());
            }
        }  else
            LOGGER.error("Missing mandatory characteristic {}",HomekitCharacteristicType.CURRENT_TEMPERATURE);
    ...
    public CompletableFuture<TargetHeatingCoolingStateEnum> getTargetState() {
        ...
        final Optional<HomekitTaggedItem> characteristic = getMandatoryCharacteristic(HomekitCharacteristicType.TARGET_HEATING_COOLING_STATE);
        if (characteristic.isPresent()) {
            stringValue = characteristic.get().getItem().getState().toString();
        } else {
    ...
    public CompletableFuture<TemperatureDisplayUnitEnum> getTemperatureDisplayUnit() {
        return CompletableFuture.completedFuture(getSettings().useFahrenheitTemperature ? TemperatureDisplayUnitEnum.FAHRENHEIT : TemperatureDisplayUnitEnum.CELSIUS);
    }
    ...
    public CompletableFuture<Double> getTargetTemperature() {
        final Optional<HomekitTaggedItem> characteristic = getMandatoryCharacteristic(HomekitCharacteristicType.TARGET_TEMPERATURE);
        if (characteristic.isPresent()) {
            DecimalType state = characteristic.get().getItem().getStateAs(DecimalType.class);
            if (state != null) {
                return CompletableFuture.completedFuture(convertToCelsius(state.doubleValue()));
    ...
    public void setTargetTemperature(Double value)  {
        final Optional<HomekitTaggedItem> characteristic = getMandatoryCharacteristic(HomekitCharacteristicType.TARGET_TEMPERATURE);
        if (characteristic.isPresent()) {
            ((NumberItem) characteristic.get().getItem()).send(new DecimalType(BigDecimal.valueOf(convertFromCelsius(value))));
        }  else {
            LOGGER.error("Missing mandatory characteristic {}", HomekitCharacteristicType.TARGET_TEMPERATURE);
        }
    }
    ...
    public void subscribeCurrentState(HomekitCharacteristicChangeCallback callback) {
        subscribeToCharacteristic(HomekitCharacteristicType.CURRENT_HEATING_COOLING_STATE, callback);
    }

    @Override
    public void subscribeCurrentTemperature(HomekitCharacteristicChangeCallback callback) {
        subscribeToCharacteristic(HomekitCharacteristicType.CURRENT_TEMPERATURE, callback);
    }

    @Override
    public void subscribeTargetState(HomekitCharacteristicChangeCallback callback) {
        subscribeToCharacteristic(HomekitCharacteristicType.TARGET_HEATING_COOLING_STATE, callback);
    }

    @Override
    public void subscribeTargetTemperature(HomekitCharacteristicChangeCallback callback) {
        subscribeToCharacteristic(HomekitCharacteristicType.TARGET_TEMPERATURE, callback);
    }
