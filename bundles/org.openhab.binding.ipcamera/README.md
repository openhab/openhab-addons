...
[
    ...
    nvrChannel=4,
    serverPort=54324,
    ffmpegOutput="/etc/openhab/html/cameras/camera-west/",
...
| `ffmpegOutput`| The full path to a unique folder (different for each camera) where FFmpeg has the ability to write files to ending with a slash. For windows use this format: `c:\openhabconf\html\cameras\camerasUID\`. If you would like to expose the GIF files to your static server, you can leave it blank which the binding will default to `/etc/openhab/html/cameras/camerasUID/` |
