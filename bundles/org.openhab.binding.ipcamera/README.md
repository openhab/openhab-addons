...
When using FFmpeg to create snapshots, it requires more CPU and you can turn this CPU load on and off via the `pollImage` channel using a switch or rule.
...
| `updateImage`| The default state of the channel `pollImage` when openHAB starts. When switched OFF (default) the image channel will NOT update unless you override this with the pollImage channel. |
