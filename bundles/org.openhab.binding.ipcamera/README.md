...
To enable TRACE logging, enter this in the openHAB console `log:set TRACE org.openhab.binding.ipcamera`, or with the log viewer add `org.openhab.binding.ipcamera` and use the drop down list to select TRACE.
...
The binding will only search using openHAB's currently selected primary network address, see Main UI>Settings>Network Settings.
