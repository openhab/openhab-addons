...
import static org.openhab.binding.emotiva.internal.EmotivaCommandHandler.clamp;
...
public class EmotivaControlRequest {
    ...
    public EmotivaControlRequest(String channel, EmotivaSubscriptionTags channelSubscription,
        ...
        if (name.equals(EmotivaControlCommands.volume.name())
            ...
            if (protocolVersion.equals(EmotivaProtocolVersion.PROTOCOL_V3)) {
                minValue = DEFAULT_TRIM_MIN_DECIBEL * 2;
                maxValue = DEFAULT_TRIM_MAX_DECIBEL * 2;
            } else {
                minValue = DEFAULT_TRIM_MIN_DECIBEL;
                maxValue = DEFAULT_TRIM_MAX_DECIBEL;
            }
        }
    }
        ...
        switch (defaultCommand.getCommandType()) {
            case NONE -> {
                switch (channel) {
                    ...
                    case CHANNEL_SOURCE -> {
                        return matchToCommandMap(ohCommand, NAME_SOURCES_MAP);
                    }
                    default -> {
            ...
            case SET -> {
                if (ohCommand instanceof StringType value) {
                    ...
                    logger.info("Could not create EmotivaControlDTO for {}:{}:{}, ohCommand is {}", channel, name, SET,
            ...
            case TOGGLE -> {
                if (ohCommand instanceof OnOffType value) {
                    ...
                    logger.info("Could not create EmotivaControlDTO for {}:{}:{}, ohCommand is {}", channel, name,
            ...
            case MODE -> {
                if (ohCommand instanceof StringType value) {
                    ...
                    try {
                        ...
                        return EmotivaControlDTO.create(getDefaultCommand());
                    }
                } else if (ohCommand instanceof Number value) {
                    if (value.intValue() >= 1) {
                        return EmotivaControlDTO.create(getUpCommand(), 1);
    ...
    private EmotivaControlDTO matchToCommandMap(Command ohCommand, String mapName) {
        if (ohCommand instanceof StringType value) {
            ...
            for (EmotivaControlCommands command : commandMap.keySet()) {
                ...
                } else if (command.name().equals(value.toString().toLowerCase())) {
        ...
        switch (dataType) {
            ...
            case DIMENSIONLESS_DECIBEL -> {
                if (name.equals(EmotivaControlCommands.volume.name())) {
                    ...
                    var doubleValue = setCommand.name().endsWith(TRIM_SET_COMMAND_SUFFIX)
                            && protocolVersion.equals(EmotivaProtocolVersion.PROTOCOL_V3)
