...
public class EmotivaProcessorHandler extends BaseThingHandler {
    ...
    private synchronized void connect() {
        ...
        try {
            ...
            if (udpSenderActive) {
                ...
                long nowEpochSecond = ZonedDateTime.now(ZoneId.systemDefault()).toEpochSecond();
                updateChannelState(LAST_SEEN_STATE_NAME, new QuantityType<>(nowEpochSecond, Units.SECOND));
    ...
    private void startPollingKeepAlive() {
        final var localRefreshJob = this.pollingJob;
        if (localRefreshJob == null || localRefreshJob.isCancelled()) {
            var keepAliveConfig = state.getChannel(EmotivaSubscriptionTags.keepAlive)
    ...
    private void checkKeepAliveTimestamp() {
        var lastKeepAliveTimestamp = Instant.EPOCH;
        if (state.getChannel(LAST_SEEN_STATE_NAME).isPresent()) {
            var lastKeepAliveValue = state.getChannel(LAST_SEEN_STATE_NAME).get();
            if (lastKeepAliveValue instanceof Number value) {
                lastKeepAliveTimestamp = Instant.ofEpochSecond(value.longValue());
            }
        }

        var deviceGoneGracePeriod = Instant.now().minus(config.keepAlive, ChronoUnit.MILLIS)
                .minus(DEFAULT_KEEP_ALIVE_CONSIDERED_LOST_IN_MILLISECONDS, ChronoUnit.MILLIS);
        var isPastGradePeriod = lastKeepAliveTimestamp.isBefore(deviceGoneGracePeriod);

        if (ThingStatus.ONLINE.equals(thing.getStatusInfo().getStatus())) {
            if (isPastGradePeriod) {
                logger.debug(
                        "Keep-alive job for '{}': status={}, last update to {} was '{}', past grace-period by '{}', scheduling connection retry",
                        thing.getUID(), thing.getStatusInfo().getStatus(), LAST_SEEN_STATE_NAME, lastKeepAliveTimestamp,
                        Duration.between(lastKeepAliveTimestamp, deviceGoneGracePeriod));
                setOfflineAndScheduleConnectRetry();
            }
        } else {
            try {
                var localConnectRetryJob = connectRetryJob;
                if (localConnectRetryJob == null || localConnectRetryJob.isCancelled()) {
                    ...
                    var currentJobDelay = Duration.of(localConnectRetryJob.getDelay(TimeUnit.SECONDS),
                            ChronoUnit.SECONDS);
                    var defaultJobDelay = Duration.of(DEFAULT_RETRY_INTERVAL_MINUTES, ChronoUnit.MINUTES);
    ...
    private void handleStatusUpdate(EmotivaUdpResponse emotivaUdpResponse) {
        ...
        if (object instanceof EmotivaAckDTO) {
            ...
            var emotivaBarNotifies = xmlUtils.unmarshallToBarNotify(answerDto.getTags());
    ...
    private void sendEmotivaUpdate(EmotivaControlCommands tags) {
        var localSendingService = sendingService;
    ...
    private void handleChannelUpdate(String emotivaSubscriptionName, String value, String visible, String status) {
        if ("keepAlive".equals(emotivaSubscriptionName)) {
            logger.trace("Subscription property '{}' with raw value '{}' received, start processing",
                    emotivaSubscriptionName, value);
        } else {
            logger.debug("Subscription property '{}' with raw value '{}' received, start processing",
                    emotivaSubscriptionName, value);
        }
        ...
        try {
            ...
            if (subscriptionTag.getChannel().equals(LAST_SEEN_STATE_NAME)) {
                ...
                logger.debug("Subscription property '{}' with value '{}' mapped to OH channel '{}'", subscriptionTag,
                        trimmedValue, subscriptionTag.getChannel());
            }
    ...
    private void findChannelDatatypeAndUpdateChannel(String channelName, String value, EmotivaDataType dataType) {
        switch (dataType) {
            ...
            case NUMBER_TIME -> {
                if (channelName.equals(LAST_SEEN_STATE_NAME)) {
                    logger.trace("Preparing to update OH channel '{}' with value:type '{}:{}'", channelName, value,
            ...
            case ON_OFF -> {
                ...
                var switchValue = OnOffType.from(value.trim().toUpperCase());
    ...
    public void handleCommand(ChannelUID channelUID, Command ohCommand) {
        ...
        if (localSendingService != null) {
            var emotivaRequest = channelToControlRequest(channelUID.getId(), state,
            ...
            if (ohCommand instanceof RefreshType) {
                ...
                try {
                    var dto = state.getChannel(channelUID.getId())
    ...
    private synchronized void disconnect() {
        final var localSendingService = sendingService;
        if (localSendingService != null) {
        ...
        final var notifyConnector = notifyListener;
        ...
        final var menuConnector = menuNotifyListener;
        ...
        var localConnectRetryJob = this.connectRetryJob;
        if (localConnectRetryJob != null) {
            localConnectRetryJob.cancel(true);
            this.connectRetryJob = null;
        }

        var localPollingJob = this.pollingJob;
