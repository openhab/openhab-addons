    ...
    private void handleChannelUpdate(String emotivaSubscriptionName, String value, String visible, String status) {
        ...
        try {
            ...
            if (subscriptionTag.getChannel().equals(LAST_SEEN_STATE_NAME)) {
                logger.trace("Subscription property '{}' with value '{}' mapped to device state '{}'", subscriptionTag,
        ...
        switch (dataType) {
            ...
            case NUMBER_TIME -> {
                ...
                if (channelName.equals(LAST_SEEN_STATE_NAME)) {
                    logger.trace("Updating device state '{}' with value '{}'", channelName, value);
                    state.updateLastSeen(now.toInstant());
                } else {
                    logger.debug("Preparing to update OH channel '{}' with value:type '{}:{}'", channelName, value,
                            Number.class.getSimpleName());
                    updateChannelState(channelName, new QuantityType<>(now.toEpochSecond(), Units.SECOND));
                }
            }
            case ON_OFF -> {
                logger.debug("Preparing to update OH channel '{}' with value:type '{}:{}'", channelName, value,
