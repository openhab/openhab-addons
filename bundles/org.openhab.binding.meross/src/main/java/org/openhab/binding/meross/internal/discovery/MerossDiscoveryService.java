...
/**
 * The {@link MerossDiscoveryService} class is responsible for performing discovery functionality
 *
...
public class MerossDiscoveryService extends AbstractThingHandlerDiscoveryService<MerossBridgeHandler> {
    ...
    private static final String PROPERTY_DEVICE_NAME = "lightName";
    ...
    public void discoverDevices() {
        MerossHttpConnector merossHttpConnector = MerossBridgeHandler.connector;
        try {
            ...
            if (devices != null && devices.isEmpty()) {
                ...
                if (devices != null) {
                    devices.forEach(device -> {
                        ...
                        deviceProperties.put(PROPERTY_DEVICE_NAME, device.devName());
                        deviceProperties.put("deviceUUID", device.uuid());
                        deviceProperties.put("deviceType", device.deviceType());
                        deviceProperties.put("firmwareVersion", device.firmwareVersion());
                        logger.debug("Found device: {} : {}", device.devName(), deviceProperties);
                        thingDiscovered(DiscoveryResultBuilder.create(deviceThing).withLabel(device.devName())
                                .withProperties(deviceProperties).withRepresentationProperty(PROPERTY_DEVICE_NAME)
