...
Discovery runs on an existing Modbus `tcp` or `serial` bridge and probes the configured endpoint using read-only register requests.
...
| writeProtectionDurationSeconds | integer | Shadow-state duration after successful write         | 5       | no       | yes      |
| autoThirdPartyControl          | boolean | Switch Solarbank into third party control on connect | true    | no       | yes      |

- The minimum value for `pollInterval` is `500` (0.5 seconds).
- When `autoThirdPartyControl` is enabled (default), Solarbank things automatically switch into `third_party_control` operating mode on connect. This is required for `battery-power-setpoint` and `battery-power-direction` writes to take effect. Set it to `false` if you want to manage the operating mode yourself. The setting is ignored for Smart Meter and Smart Plug and EV Charger things.

## Bridge Configuration

The Anker SOLIX thing inherits connection settings from that bridge and only provides device-specific parameters.

This still allows one endpoint per device: create one Modbus bridge per device (or gateway endpoint), each with its own host/port settings.
...
1. If discovery does not detect your device, add the Anker SOLIX thing (`ankersolix-*`) manually as child of that bridge.
...
- `*` marks capability-dependent channels. Availability and write support depend on the optional capability mask, so depending on firmware/device these channels can be unsupported (`UNDEF`) or reported as read-only.
- Devices that do not provide the capability mask register are handled with fail-open defaults, and openHAB may log a small number of expected startup `WARN`/`ERROR` entries for that single register before polling of that register is stopped; see [Troubleshooting](#troubleshooting) for details.
