...
public class TimeInfoMsg extends BaseMsg {
    ...
    private static final int TIME_INFO_PAYLOAD_LEN = 5; /* in bytes */
    private static final int TIME_INFO_REQUEST_PAYLOAD_LEN = 0;

    private final Logger logger = LoggerFactory.getLogger(TimeInfoMsg.class);

    private @Nullable Calendar messageTimeInfo;
    private TimeZone tz = TimeZone.getTimeZone("Europe/London");

    public TimeInfoMsg(String rawMsg) throws MaxCulProtocolException {
        super(rawMsg);
    ...
    protected void printFormattedPayload() {
        super.printDebugPayload();
        @Nullable
        Calendar messageTimeInfo = this.messageTimeInfo;
        if (messageTimeInfo != null) {
            logger.debug("\tDecoded Time: {}-{}-{} {}:{}:{}", messageTimeInfo.get(Calendar.YEAR),
                    messageTimeInfo.get(Calendar.MONTH + 1), messageTimeInfo.get(Calendar.DAY_OF_MONTH),
                    messageTimeInfo.get(Calendar.HOUR_OF_DAY), messageTimeInfo.get(Calendar.MINUTE),
                    messageTimeInfo.get(Calendar.SECOND));
        }
    }

    public @Nullable Calendar getTimeInfo() {
