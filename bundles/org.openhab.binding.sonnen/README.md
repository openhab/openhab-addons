...
| batteryChargingFromGrid        | Switch        | read/write | Starts/stops active battery charging from the Grid. Note: "Write-API" must be activated in the local web interface, and the token must be entered in the binding's authentication field.                                                       |
| batteryDischargingToGrid       | Switch        | read/write | Starts/stops active battery discharging to the Grid. Note: "Write-API" must be activated in the local web interface, and the token must be entered in the binding's authentication field.                                          |
