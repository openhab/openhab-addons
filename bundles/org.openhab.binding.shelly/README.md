...
Once the initialization is triggered the Thing switches to Status UNKNOWN, CONFIGURATION_PENDING.  
Bayttery powered devices like HT or BLU series stay in this status until they receive an event, then the thing will change to ONLINE status.    |
| UNKNOWN        | Indicates that the status is currently unknown, which must not show a problem.
Usually the Thing stays in this status when the device is in sleep mode.
