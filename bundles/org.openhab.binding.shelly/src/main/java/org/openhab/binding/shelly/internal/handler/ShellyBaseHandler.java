...
public class ShellyBaseHandler extends BaseThingHandler implements ShellyDeviceListener {
    ...
    @Nullable
    private ShellyCoapHandler coap;
    @Nullable
     ...
     * @param networkAddressService instance of NetworkAddressService to get access
     *            to the OH default ip settings
    ...
    public ShellyBaseHandler(Thing thing, ShellyHandlerFactory handlerFactory, ShellyBindingConfiguration bindingConfig,
            @Nullable NetworkAddressService networkAddressService, @Nullable ShellyCoapServer coapServer) {
        super(thing);

        Validate.notNull(handlerFactory);
        Validate.notNull(bindingConfig);
        Validate.notNull(networkAddressService);
    ...
    protected void initializeThingConfig() {
        ...
        config.localIp = lip == null ? "0.0.0.0" : lip;
    ...
    public void onEvent(String deviceName, String deviceIndex, String type, Map<String, String> parameters) {
        if (thingName.equalsIgnoreCase(deviceName) || config.deviceIp.equals(deviceName)) {
            ...
            String channel = group + CHANNEL_GROUP_SEPARATOR + CHANNEL_EVENT_TRIGGER;
