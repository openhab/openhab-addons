...
| `apparentPower` | `Number:Power` | VA | Apparent power (data-driven; absent if device does not report it) |
| `voltage` | `Number:ElectricPotential` | V | Phase voltage |
| `current` | `Number:ElectricCurrent` | A | Phase current |
| `frequency` | `Number:Frequency` | Hz | Line frequency (data-driven; absent if device does not report it) |
...
| `accumulatedWatts` | `Number:Power` | W | Device-level instantaneous active power (sum of all meters) |
| `accumulatedApparent` | `Number:Power` | VA | Device-level apparent power sum (Pro 3EM / Pro EM-50 only) |
| `accumulatedReturned` | `Number:Energy` | kWh | Device-level total returned energy (Pro 3EM / Pro EM-50 only) |
...
|---|---|---|
| `meterN#reactiveWatts` | `meterN#reactivePower` | Unit changed: W → VAR |
| `device#accumulatedWTotal` | `device#totalEnergy` | Pro 3EM; requires re-discovery |
| `device#accumulatedKWHTotal` | `device#totalEnergy` | Relay-PM (Plus 2PM, Plus 1PM, Plus Strip 4 Gen4); requires re-discovery |
| `device#totalKWH` | `device#totalEnergy` | Relay-PM; `device#totalKWH` cannot be auto-migrated (name collision with `meter#totalKWH`); requires re-discovery |
| `meterN#powerFactor` type | `Number` (was `Number:Dimensionless`) | Range −1.0 to +1.0; persistence / UoM links break |
| `meterN#lastPower1` | `meterN#lastEnergy1` | Auto-migrated; no re-discovery needed |
