 ...
 */
@NonNullByDefault
final class ChannelActuatorLimitation extends VeluxChannelHandler {
    ...
    static @Nullable State handleRefresh(ChannelUID channelUID, String channelId,
        ...
        do { // just for common exit
            ...
            GetProductLimitation bcp = thisBridgeHandler.thisBridge.bridgeAPI().getProductLimitation();
            if (bcp == null) {
                LOGGER.trace("handleRefresh(): aborting processing as handler is null.");
                break;
            }
            bcp.setActuatorIdAndLimitationType(thisProduct.getBridgeProductIndex().toInt(), true);
            if (thisBridgeHandler.thisBridge.bridgeCommunicate(bcp)) {
                int limitation = bcp.getLimitation();
                if (bcp.isCommunicationSuccessful()) {
                    try {
                        VeluxProductPosition position = new VeluxProductPosition(limitation);
                        if (position.isValid()) {
                            PercentType positionAsPercent = position.getPositionAsPercentType(isInverted);
                            LOGGER.trace("handleRefresh(): found limitation of actuator at level {}.",
                                    positionAsPercent);
    ...
    static @Nullable Command handleCommand(ChannelUID channelUID, String channelId, Command command,
        ...
        LOGGER.warn("handleCommand(): not yet implemented.");
