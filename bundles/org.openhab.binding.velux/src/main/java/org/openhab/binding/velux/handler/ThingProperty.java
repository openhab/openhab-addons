...
final class ThingProperty {
    ...
    static boolean exists(BaseBridgeHandler bridge, ChannelUID channelUID, String propertyName) {
        LOGGER.trace("exists({},{}) called.", channelUID, propertyName);
        ...
        if (thingOfChannel == null) {
            LOGGER.trace("exists(): Channel {} does not belong to a thing.", channelUID);
        } else {
            if (thingOfChannel.getConfiguration().get(propertyName) != null) {
                exists = true;
            }
        }
        LOGGER.trace("exists() returns {}.", exists);
        return exists;
    }
    ...
    static Object getValue(BaseBridgeHandler bridge, ChannelUID channelUID, String propertyName) {
        LOGGER.trace("getValue({},{}) called.", channelUID, propertyName);
        ...
        LOGGER.trace("getValue() returns {}.", propertyValue);
    ...
    static void setValue(ExtendedBaseBridgeHandler bridgeHandler, ChannelUID channelUID, String propertyName,
        ...
        LOGGER.trace("setValue({},{}) called.", propertyName, propertyValue);
        ...
        thingOfChannel.setProperty(propertyName, propertyName);
        LOGGER.trace("setValue() done.");
        return;
    }
