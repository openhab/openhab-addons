    ...
    public void setResponse(short responseCommand, byte[] thisResponseData, boolean isSequentialEnforced) {
        ...
        switch (Command.get(responseCommand)) {
            ...
            case GW_GET_NODE_INFORMATION_NTF:
                ...
                int[] ntfFuntionalParameters = new int[VeluxBindingConstants.FUNCTIONAL_PARAMETER_COUNT];
                for (int i = 0; i < ntfFuntionalParameters.length; i++) {
                    ntfFuntionalParameters[i] = responseData.getTwoByteValue(89 + (i * 2));
                }
                ...
                if (logger.isTraceEnabled()) {
                    ...
                    for (int i = 0; i < ntfFuntionalParameters.length; i++) {
                        logger.trace("setResponse(): ntfFunctionalParameter{}={}.", i + 1, ntfFuntionalParameters[i]);
                    }
