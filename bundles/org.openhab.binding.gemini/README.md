...
| toolLoopLimit   | integer | The maximum number of tool execution loops allowed during a single interpretation.                                                       | 15                    | no       | yes      |
...
The used model, temperature, topP, maximum output tokens and tool loop limit parameters can be configured in the thing configuration.
...
```java
Thing gemini:account:myaccount [apiKey="xxx-yyy-zzz", model="gemini-3.1-flash-lite", temperature=1.0, topP=1.0, maxOutputTokens=2048, toolLoopLimit=15, requestTimeout=30] {
  Channels:
