 ...
 */
package org.openhab.binding.netatmo.internal.api.security;
...
import org.openhab.binding.netatmo.internal.api.home.HomeApi.NAHomesDataResponse;
...
public class SecurityApi extends RestManager {
     ...
     *             response body
     */
    public boolean addwebhook(String url) throws NetatmoException {
        String req = "addwebhook?url=" + url;
        ApiOkResponse response = post(req, null, ApiOkResponse.class, true);
        if (!response.isSuccess()) {
            throw new NetatmoException(String.format("Unsuccessfull schedule change : %s", response.getStatus()));
        }
