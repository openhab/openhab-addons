...
public class NAPresenceCameraHandlerTest {
    ...
    private Bridge presenceCameraThing;
    private NAWelcome presenceCamera;
    ...
    @BeforeEach
    public void before() {
        presenceCameraThing = new BridgeImpl(new ThingTypeUID("netatmo", "NOC"), "1");
        presenceCamera = new NAWelcome();

        cameraStatusChannelUID = new ChannelUID(presenceCameraThing.getUID(),
                NetatmoBindingConstants.CHANNEL_CAMERA_IS_MONITORING);
    ...
    // }
    //
    private class NAPresenceCameraHandlerAccessible extends NAPresenceHandler {

        private NAPresenceCameraHandlerAccessible(Bridge thing, NAWelcome presenceCamera) {
            super(thing, List.of(), null, timeZoneProviderMock);
            setNAThing(presenceCamera);
        }
