...
public class NAWelcomeHomeHandlerTest {
    ...
    @BeforeEach
    public void before() {
        Bridge welcomeHomeThing = new BridgeImpl(new ThingTypeUID("netatmo", "NAHomeSecurity"), "1");
        handler = new NAWelcomeHomeHandlerAccessible(welcomeHomeThing);
    }

    @Test
    public void testUpdateReadingsWithEvents() {
        NAHomeEvent event1 = createEvent(1592661881, EventType.PERSON);
        NAHomeEvent event2 = createEvent(1592661882, EventType.MOVEMENT);

        NAHome home = new NAHome();
        // home.setId(DUMMY_HOME_ID);
        home.setEvents(Arrays.asList(event1, event2));

        NAHomeData homeData = new NAHomeData();
    ...
    // }
    //
    private static NAHomeEvent createEvent(int eventTime, EventType eventType) {
        NAHomeEvent event = new NAHomeEvent();
        event.setEventType(eventType);
        event.setTime(eventTime);
        return event;
    }

    private class NAWelcomeHomeHandlerAccessible extends NAHomeSecurityHandler {

        private int triggerChannelCount;
        private String lastDetectedObject;

        private NAWelcomeHomeHandlerAccessible(Bridge thing) {
            super(thing, List.of(), null, timeZoneProviderMock);
        }
